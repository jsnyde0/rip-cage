...
- **Three cage skills replace `configure-cage` and the vendored microsandbox copy** (rip-cage-ely4.13; [ADR-031](docs/decisions/ADR-031-opinionated-distribution-of-microsandbox.md) D6). `cage-config` writes the one native msb config a project launches from, `cage-image` writes the Dockerfile that extends the base image, `cage-ops` runs and troubleshoots a live cage — each with its own `recipes/` and `references/`. `configure-cage` taught the retired `tools.yaml` manifest and the retired layered config, so it was replaced rather than edited. `.claude/skills/microsandbox/` was a diverged COPY of the upstream skill; the three new skills point at `~/code/personal/superradcompany-skills/microsandbox/SKILL.md` by path instead, because a copy drifts from what it copied. Earlier entries in this file that name `configure-cage` describe what shipped at the time and are left as written.
- **`rc destroy` no longer picks a cage nobody named** (rip-cage-ely4.7.13). Its name resolution ended in singleton auto-select, so an empty name — from a failed lookup, say — removed whatever single rc-managed cage existed, with its named volumes. It now takes the name you typed or the cage the current directory names, and otherwise refuses with exit 2, listing the cages it left standing. The read-only verbs keep auto-select.
- **The host Claude config reaches a cage through the cage config, read-only** (rip-cage-ely4.7.10; [ADR-031](docs/decisions/ADR-031-opinionated-distribution-of-microsandbox.md) D2, [ADR-024](docs/decisions/ADR-024-prompt-injection-threat-model.md)). `rc up` used to add this mount itself, and read-write, since the switch that chose the mode was pinned to one value when its schema retired. It is now one line in `share/rip-cage/cage.yaml.template` — visible to anyone reading the config, and `:ro`, because init snapshots the file to a seed at boot and read-write would expose the `mcpServers`/`hooks` that the HOST Claude later executes.

## [0.13.1] - 2026-07-29

Post-cutover stabilization for the msb migration, driven largely by the first real dogfooding run (the maintainer's own factory cage moved onto msb, surfacing the docker-era remnants that survived the hard cutover). Two additive layers land alongside the fixes: **[ADR-030](docs/decisions/ADR-030-classify-by-use-secret-posture.md)'s classify-by-use secret posture** — a `mounts.mask` primitive plus a documented Class-A non-possession recipe, structured as an opt-in gradient with a zero-work default — and **[ADR-029](docs/decisions/ADR-029-msb-migration.md) D8's herdr roster-resume recipe**, which makes `rc reload`'s cold-recreate (and a hard VM crash) survivable for multi-agent sessions, conversations and all.

### Added

- **`mounts.mask` — declarative workspace masking** (rip-cage-goaz; [ADR-030](docs/decisions/ADR-030-classify-by-use-secret-posture.md) D4–D7). Operator-declared workspace-relative paths are masked in-cage at `rc up` via a single-file `:ro` overmount of a legible rc-owned breadcrumb ("masked by rip-cage") — the same mechanics as the in-production `config_mode: ro` shadow-mount. This is Tier 1 of ADR-030's gradient: a repo's Class-C secrets (real files the caged task simply doesn't need) stop being readable by a prompt-injected agent, without disturbing the default posture (the list defaults empty — nothing masks unless you say so). Lists union across config layers with provenance in `rc config show`; a missing source aborts loud; absolute or `..`-escaping paths are rejected; single-file only (msb `--mount-file` is regular-files-only). No scanner and no auto-wirer — which paths are Class-C is judgment, not machinery ([ADR-005](docs/decisions/ADR-005-ecosystem-tools.md) D12).
- **[ADR-030](docs/decisions/ADR-030-classify-by-use-secret-posture.md) + `docs/reference/secret-posture.md` — project secrets classified by use, not by type** (rip-cage-g5jg, rip-cage-m613). Tier 0 (the default, unchanged) is the cage plus the egress wall; Tier 1 is `mounts.mask`; Tier 2 is per-credential non-possession for Class-A wire-bearers, riding the existing `auth.credentials` → msb `--secret` path ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D5). The new reference doc carries the Tier-2 recipe with a worked example, Class-A membership heuristics, the substitution dead zones (base64/body/query, mTLS/pinning), and the criterion for reaching for Tier 2 at all (high-value **and** leak-cost great/irreversible — deliberately not an enumerated credential-type list). Docs and skill only; zero rc code. One residual probed and recorded rather than hand-waved: msb 0.6.4 scrubs secrets outbound-only, so a bound host that *reflects* a request body can echo a substituted credential back into the guest (rip-cage-x640; upstream FR drafted at rip-cage-yk3s, interim guidance in the doc).
- **`configure-cage` gains a secret-posture sweep** (rip-cage-3npt). The skill can now scan a workspace — or a pooled tree of dozens of repos, the case that made per-repo hand-rework untenable — and propose, for human approval, a `mounts.mask` list plus Tier-2 flags for anything meeting the ADR-030 D3 criterion. Skill-doc only: the human approves, the agent writes, rc stays dumb.
- **herdr roster-resume — caged sessions survive a cold `rc reload` and a hard crash** (rip-cage-46s5; [ADR-029](docs/decisions/ADR-029-msb-migration.md) D8). Recipe-level only, with zero rc-core or MULTIPLEXER-contract edits: `examples/herdr/` now mounts a per-cage host directory at `~/.config/herdr` so the roster outlives the recreate, relocates `HERDR_SOCKET_PATH` to a guest-local path, and bakes a headless PTY `scripted-attach.py` that triggers herdr's native roster restore — which does not fire on server start alone, only on a client attach. herdr pin v0.7.0 → v0.7.5. The claude session wrapper additionally scrubs the inherited `CLAUDE_CODE_CHILD_SESSION` marker, which was silently disabling transcript saving in interactive panes. Proven end-to-end with two caged claude sessions and planted codewords: both a `kill -9` of the VM and a graceful `rc reload` recover the layout, the session ids, and the conversations.
- **Pre-reload guard against silent transcript loss** (rip-cage-aa4t). `rc reload` cold-recreates, destroying the guest's ephemeral overlay — so a cage predating the now-unconditional host-bind of `~/.claude/projects` would lose in-flight conversations while herdr faithfully restored the pane layout. `rc reload` now refuses loud *before* the recreate when a cage's transcripts are not host-bound (override `--allow-transcript-loss`; `--dry-run` reports what the guard would do without refusing or mutating), and `rc doctor` gains a matching transcript-persistence probe — OK/WARN/INFO, never FAIL, because this is a legacy edge state, not a broken cage.

### Changed

- **`/etc/rip-cage/release` is the blessed in-guest cage marker, replacing `/.dockerenv`** (rip-cage-r5f9; [ADR-002](docs/decisions/ADR-002-rip-cage-containers.md) D14). msb microVM guests have no Docker runtime at all, so the `/.dockerenv` check silently stopped firing at cutover and `rc` no longer refused to run *inside* a cage. Every built image now bakes a root-owned (`root:root`, 0444) sentinel; root ownership is the load-bearing part — an agent-writable marker would be forgeable by a prompt-injected agent.
- **Cage `ssh_config` pins host keys from the system floor only** (rip-cage-5iti). `UserKnownHostsFile` drops its user-path `~/.ssh/known_hosts` entry: the per-cage filtered `known_hosts` mount it pointed at retired with the ssh cluster ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D3 / [ADR-022](docs/decisions/ADR-022-ssh-allowlist.md) D6), leaving only the image-baked `/etc/ssh/ssh_known_hosts` github.com floor load-bearing. Git authenticates over HTTPS + `--secret`; this config now only pins host keys for any ssh someone composes on top. Safety floor re-proven under msb on a throwaway cage (65/66, the one fail environmental).

### Fixed

- **Caged claude no longer shows the bypass-permissions accept dialog on every cold boot** (rip-cage-k8vi). The dialog blocked every restored or spawned agent pane — a direct autonomy regression, since a walk-away cage cannot answer it. The claude session wrapper now idempotently prepends `--dangerously-skip-permissions` to the exec'd argv at the real launch point (only when the caller hasn't already supplied it), which kills the dialog independently of *why* it was appearing; it also seeds `bypassPermissionsModeAccepted` into the writable per-session `.claude.json` as belt-and-suspenders. Policy-aligned, not a weakening: `settings.json` already declares `defaultMode: bypassPermissions` — the flag just states that at argv level. The wrapper is the single PATH-shadowing chokepoint, so every launch path (herdr start/restore, human `claude`, scripted-attach `claude --resume`) is covered without special-casing a multiplexer.
- **`rc attach` works against msb cages with a baked multiplexer** (rip-cage-vjuv). Two docker-era remnants the factory-cage dogfood surfaced: the multiplexer-registry existence check (`_rc_mux_resolve_hook_path`) still ran `docker exec`, so attach fail-louded "no baked multiplexer registry for herdr" — with a stale `docker inspect` hint — even though the registry *was* baked; and the attach hook didn't export the relocated `HERDR_SOCKET_PATH`, so a bare `herdr` attach fell back to the empty default socket and died with `Os NotFound`.
- **`rc allowlist add <host> --cage <name>` edits the cage's workspace config, not CWD's** (rip-cage-e25p). Double silent failure: run from `~`, it created a stray `~/.rip-cage.yaml` while the cage's real config went untouched, and the auto-reload then reported "No changes" because `rc reload`'s diff reads the cage's recorded workspace. The edit target is now resolved from the cage's `rc.source.path` label — the same source the reload diffs — and `--cage` combined with a *divergent* `--config-file` fails loud before writing anything.
- **`rc doctor` / `rc reload --dry-run` never suggest allowlisting a host a credential was blocked from reaching** (rip-cage-jlu4). msb writes DNS denials and secret-violation blocks (a substituted placeholder caught heading to a disallowed host — a caught exfil) to the same trace log, indistinguishably in-guest. The fix-hint miner surfaced everything as "add `<host>` to the allowlist", which for a secret violation would have talked an operator into allowing the very host the misdirection was blocked toward. A disjoint second miner now reports secret violations as their own distinct warning, never routed through the allowlist-suggestion path.
- **Shell completions enumerate cages via msb, not a dead `docker ps`** (rip-cage-tsf2.5). Both the bash and zsh completions shelled out to `docker ps` for dynamic cage-name completion; no docker daemon backs cages post-cutover. They now call `rc --output json ls`, filtered to running cages for `attach`/`exec`/`down`/`test`/`reload` and unfiltered for `destroy`/`doctor` — mirroring the old `docker ps` vs `docker ps -a` split.
- **The cage-facing test suite runs on msb; the host-only CI job installs msb** (epic rip-cage-neu7, rip-cage-im8q). Eighteen test files still resolved, exec'd into, and cleaned up cages through `docker ps|exec|inspect|cp|rm` — all invisible to msb-created sandboxes — and CI's host-only job never installed `msb`, chronically redding three `rc up` tests. Cleanup traps were rebuilt fail-safe in the same pass: they now destroy only cage names explicitly registered at creation, replacing an enumerate-all + label-glob shape that destroyed a live daily-driver cage when a staging variable came up empty.
- **Post-cutover documentation sweep** (rip-cage-o9b4, rip-cage-ffmc, rip-cage-9xll, rip-cage-f1qo). CLAUDE.md, the reference docs, the examples, and in-code comments now describe msb reality instead of the Docker era; `adding-a-tool.md` no longer understates a manifest's `egress` field as documentation-only (declared hosts are IOC-denylist-checked at build/up/reload *and* unioned into the cage's runtime allowlist as msb net-rules); and the one egress denial with no fix-hint is now documented — msb logs nothing at TCP-connect for a right-domain-wrong-port drop, so it surfaces client-side as a plain `Connection refused`, with `rc doctor` and `docs/reference/egress.md` pointing back at the allowed **port** in `.rip-cage.yaml`.

## [0.13.0] - 2026-07-23

The headline is the **msb migration**: rip-cage re-platforms its isolation primitive from Docker/OrbStack (a co-located-uid boundary inside one shared kernel) onto **microsandbox (msb, libkrun microVMs)** — a host/VM boundary ([ADR-029](docs/decisions/ADR-029-msb-migration.md)). Docker is now a **build-time-only** tool (`cage/Dockerfile` via `docker build`); msb loads and runs the built image, not `docker run`/`docker exec`. The in-cage security engine (SNI router, iptables REDIRECT chains, DNS resolver sidecar, TLS-intercept mediator launch) is deleted, not ported — egress destination control, DNS default-deny, and credential non-possession are now consumed as msb host-side runtime primitives that rc *declares* against, rather than an engine rip-cage maintains. Alongside the migration, `.rip-cage.yaml` moves to config-schema v2.

### Changed (breaking)

- **microsandbox (msb) replaces the Docker runtime as the isolation boundary** (epic rip-cage-tsf2; [ADR-029](docs/decisions/ADR-029-msb-migration.md) D1/D2 — hard cutover, no dual backend). `rc build`/`up`/`attach`/`exec`/`down`/`destroy`/`reload`/`ls`/`doctor`/`test` are rewritten onto msb's `msb run`/`msb load`/lifecycle verbs. Docker is retained only to build the OCI image (`docker build` → `docker save` → `msb load`); there is no longer a Docker-runtime containment boundary. The welded containment floor is now: the microVM boundary itself, msb default-deny egress + net rules, msb DNS default-deny, `--secret` credential non-possession, plus the surviving host-mount floor items msb doesn't subsume (the ADR-023 secret-path mount denylist, the ADR-002 D11 `.git/hooks` read-only weld) and the surviving in-guest floor items (root-owned guard artifacts, scoped sudo, ro mounts).
- **Default-deny egress at the VM boundary + a curated host allowlist** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D4). Observe-mode retires — msb logs nothing for allowed flows, so rebuilding allow-observation would mean rebuilding the deleted engine. Replacement: a curated default allowlist declared in `.rip-cage.yaml` under `network.allowed_hosts` (`api.anthropic.com`, `github.com`, package registries, …), each default-floor host scoped **port-tight** to `tcp:443` unless the entry names an explicit port (rip-cage-mzu6; a Linux/KVM CI proof landed as a permanent regression guard, rip-cage-4fxg), and a fast **deny→fix→reload repair loop**: `rc doctor`/`rc reload`'s dry-run mine the sandbox's trace-level DNS-denial log for a fix-hint instead of the operator having to guess the exact hostname.
- **`rc reload` is now a cold-recreate, not a hot-reload** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D4, rip-cage-rj68). msb's net rules have no live-mutation path on a running sandbox (`msb modify` carries no network parameter), so `rc reload` runs graceful-stop → remove → the same create pipeline `cmd_up` uses, against the now-current `.rip-cage.yaml`. **Survives the recreate:** host mounts (the workspace, `~/.claude/{projects,sessions}` — the Claude session **resumes**, it is not lost) and named volumes (`rc-state-*`, `rc-history-*`, `rc-mise-cache`). **Lost:** only the guest's own ephemeral rootfs overlay scratch (e.g. an ad-hoc `apt-get install` at runtime not baked into the image or captured by a mount) — a narrow, documented tradeoff, not a session-continuity loss.
- **`rc up` converges on drift by default on stopped cages** (rip-cage-y0u0). A stopped cage whose config has drifted since creation now converges (recreate-with-current-config) as `rc up`'s default behavior, superseding the prior explicit `--reload`/`RC_UP_CONVERGE` opt-in.
...
- **Git authenticates over HTTPS with a per-cage token injected by msb `--secret`** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D3, rip-cage-7fqe). Cages now push/pull/`gh` as `https://x-access-token:$TOKEN@...`, with the token injected by msb's `--secret VAR@HOST` placeholder-substitution primitive — proven end-to-end (clone, commit, push, `gh api`, `gh pr create`). The per-cage token is the identity scope; the guest never holds the real value, including in `.git/config`.
- **Credential non-possession via msb-native `--secret` is now a default platform property, not opt-in** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D5, rip-cage-cmqb, rip-cage-9dlw). The real Claude Code CLI runs fully non-possessed by default: guest env/disk/proc hold only a placeholder, msb injects the real token on the wire toward the bound host only, and a violation guard blocks-and-logs the placeholder toward any unbound host. `auth.credentials` entries carry `target_env` (guest env-var(s) receiving the placeholder) and `source_file` (a host-side token file feeding `--secret`), so a plain `rc up` needs no pre-exported host env var. pi's openai-codex provider still has no static token, so pi stays possession-mode — mixed posture remains the caged-pi shape under the new mechanism.
...
- **The ssh cluster and the ssh-bypass command-guard sibling retire** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D3, epic rip-cage-tsf2). Git's canonical in-cage path is now HTTPS + `--secret` (see Added above), so ADR-017 (agent-forwarding default), ADR-018 (socket discovery), ADR-020 (identity routing), and ADR-022 (host+key allowlist, `ssh-agent-filter`, filtered `known_hosts`, `block-ssh-bypass.sh`, `examples/ssh-bypass/`) all retire with the deleted in-cage engine; the `rc config init` ssh-detection wizard goes with them. The LAN-IP ssh-agent bridge (`rip-cage-akg5`) remains documented as a composed example recipe only, never blessed — it forwards the whole host agent with no per-key scoping, strictly worse than per-cage tokens as a default.
- **The in-cage security engine** ([ADR-029](docs/decisions/ADR-029-msb-migration.md) D2). `rip_cage_router.py`, `rip_cage_egress.py`, `rip_cage_dns.py`, `init-firewall.sh`, the iptables REDIRECT machinery + legacy-iptables pin, `init-mediator.sh`, and the MEDIATOR archetype/launch machinery are deleted, not ported — msb's own default-deny egress, default-deny DNS, and `--secret` (see Changed/Added above) take over the properties this engine used to provide in-guest. The composed-mediator path (ADR-026) remains valid as an opt-in escape hatch for cases `--secret` can't express.
