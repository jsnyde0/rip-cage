...
- **Built for the agentic era — agents install, configure, and compose; rip-cage does not script it.** In rip-cage, software is set up, configured, and wired together *by agents*, not by deterministic installers, setup scripts, or config-merge directives. The job is therefore to make recipes **discoverable and legible to an agent** (clear example fragments under `examples/`, obvious seams, good headers/READMEs) — NOT to build machinery that automates composition. **Design tripwire:** if you are about to add a `compose:` directive, an install script, a fixed config-merge step, an auto-wiring mechanism, or any deterministic plumbing whose job is "tie these pieces together for the user," STOP — the agent configuring the cage does that wiring itself by reading the recipe. Provide the seam and the legible recipe; let the agent compose (the existing "copy these manifest entries, then `rc build`" flow is already correct — an agent does it trivially). This is a distinct principle from "composable seam, not a bundler" above and it is the one agents (and humans pattern-matching on traditional software) *keep* drifting back from — every re-proposed `compose:`/installer/auto-wire mechanism is this drift. Canonical: [ADR-005 D12](docs/decisions/ADR-005-ecosystem-tools.md) (agentic-composition premise). Hold it.
