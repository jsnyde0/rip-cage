...
- [ADR-030 Classify-by-Use Secret Posture](ADR-030-classify-by-use-secret-posture.md) (D2 exfil axis evolved in place — two content-level layers for project secrets, Class-C masking + Class-A non-possession, plus the reflection residual)
...
- **Exfiltration:** sensitive information leaves the cage. Targets include credentials, workspace contents, host files reachable via mounts (covered by ADR-023 mount denylist), **project secrets sitting in the mounted workspace tree** (content-level layers per ADR-030 — Class-C masking / Class-A non-possession — atop the egress-firewall row), and host files reachable via ssh-agent forwarding (covered by ADR-017 evolution).
...
| Class-C masking (ADR-030 D4, Tier 1 `mounts.mask`) — **content-level; opt-in per path, default empty** | strong for *unused* project secrets in the mounted tree (masked content is not readable in-cage, so cannot be exfiltrated); scoped to declared Class-C paths only | n/a |
| Class-A non-possession (ADR-030 Tier 2, reuses ADR-029 D5 `--secret`) — **content-level; opt-in per credential, never default for project secrets** | strong for wire-bearer *project* secrets (real value never in-guest), **with the reflection residual below** | n/a |

> [ADR-030, 2026-07-27: EVOLVED — the exfil axis gains **two content-level layers** for *project* secrets in the mounted workspace tree, which were previously defended by the egress-firewall row alone: **Class-C masking** (ADR-030 D4 boot-time `ro` overmount) reduces what is readable in-cage, and **Class-A non-possession** (ADR-030 Tier 2, the ADR-029 D5 `--secret` mechanism generalized to a project credential) keeps a wire-bearer's real value off the guest. Both are opt-in (ADR-030 D2's zero-work-default gradient); neither is default for project secrets. **Reflection residual (observed live, spike S2, `rip-cage-x640`):** a bound host that *reflects* the substituted real value in its response body defeats Class-A non-possession — the guest reads the real value back, and the violation guard (which keys on the placeholder) is blind, so the reflected value can then ride egress to any allowlisted host. Prompt-injection-relevant: an injected agent could deliberately elicit reflection via error pages / echo endpoints. Probe complete (`rip-cage-x640`, 2026-07-27): msb 0.6.4 has **no** response-path scrubbing — verified in the upstream secret-handler source (substitution is outbound-only) — so the gap is genuine, not config-closable. Interim operational guidance is in `docs/reference/secret-posture.md`; an upstream feature request (Daytona-style response scrubbing) is drafted and tracked in `rip-cage-yk3s`, pending human authorization to file. This residual is part of D2's threat surface, not a closed layer.]
