...
| [005](ADR-005-ecosystem-tools.md) | Ecosystem Tools Integration | toolchain | Proposed (revised 2026-06-14) | Build-time (not runtime) tool composition — the originally-specified build-arg layer (INCLUDE_ toggles, versions.env, --with flags) was never built and is superseded by the host-only manifest; bundled-tool versions are hardcoded Dockerfile ARG defaults; D7–D10 composable host-only tool manifest (3 archetypes, in-cage-only, floor-untouchable, fail-warn daemons); D11 agent-first composability (generic from-source builder stage + fail-closed validator + host-side authoring skill); D2 bundles only what's integral to operating the cage (cm demoted to manifest worked example); **D12 (2026-06-15, FIRM) rip-cage is a composable seam, not a bundler — rc never names/bundles/blesses an optional tool, defaults ship minimal, examples live outside the binary; D9 clarified (multiplexer provider hooks are availability-payload) + D11 validator bounds them** |
