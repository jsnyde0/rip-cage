...
| [006](ADR-006-multi-agent-architecture.md) | Multi-Agent Architecture | multi-agent | Proposed (revised 2026-06-13) | Progressive tiers; D7 re-decided (rip-cage-1f59): the Tier-1a mechanical lever leaves rip-cage for the composed in-cage multiplexer (`session.multiplexer: none\|tmux\|herdr`, default none) — `rc agent`/`rc sessions` removed; rip-cage owns box-entry (`rc up`/`attach`/`exec`), the multiplexer owns spawn/list/supervise; orchestration still lives in the consumer; concurrency agent-specific (pi safe, claude via rip-cage-p1p) |
