...
- Files mounted: the generated config (one file), each `<keyname>.pub` referenced by an `IdentityFile` directive (zero or more), and a **filtered** slice of `~/.ssh/known_hosts` (per [ADR-022](ADR-022-ssh-allowlist.md) `ssh.allowed_hosts`; absent allowlist defaults to an empty filtered file → cage-baked `/etc/ssh/ssh_known_hosts` github.com pins are the floor). Wholesale mount of `~/.ssh/known_hosts` was the original v1 design and was bypassable via `-o UserKnownHostsFile=~/.ssh/known_hosts` per ADR-014 D2's line-79 caveat; ADR-022 closes that at the mount layer.
...
5. **Override ADR-014 D2 directives at per-Host level.** If user config sets `BatchMode no` or `StrictHostKeyChecking accept-new` inside a `Host` block, those values would defeat the cage's TTY-hang fail-loud posture (the `Match final Host *` system block at `/etc/ssh/ssh_config.d/00-rip-cage.conf` does not override per-Host user values per OpenSSH "first value wins"). Translation rewrites these two to ADR-014 D2 values with `# rip-cage: overridden (ADR-014 D2)` comment. **`UserKnownHostsFile` and `GlobalKnownHostsFile` are no longer rewritten** — that defense was bypassable via CLI `-o` and was theater on top of ADR-020 D1's wholesale mount; per [ADR-022](ADR-022-ssh-allowlist.md), host trust is scoped at the mount layer (filtered `~/.ssh/known_hosts`) where no `-o` flag can re-mount the file. The `BatchMode` and `StrictHostKeyChecking` overrides remain because they have no `-o`-equivalent attack worth mitigating: a per-Host `BatchMode no` is a config hygiene issue, not a containment-bypass attempt.
...
**`--no-forward-ssh` implies `--no-ssh-config` by default.** A user reaching for ADR-014 containment finds `--no-forward-ssh`; without the implicit chain, they'd end up with a cage that has translated config + pubkey mounts but no agent — confusing posture ("I asked for no SSH, why does my cage have my pubkey files?"). Mirrors the `_UP_NO_HOST_AGENT` auto-relabel pattern as it stood then. **Note added 2026-09-14 (rip-cage-o5be):** that precedent is gone. Host ssh-agent forwarding retired with the rest of the ssh cluster (ADR-029 D3; `cli/ls.sh:'are RETIRED labels'`), and `_UP_NO_HOST_AGENT` is absent from the tree — `grep -rn '_UP_NO_HOST_AGENT' rc cli/` returns nothing. The decision does not stand without it, and is not meant to: D7 is itself retired per the banner above. Unlike D3 and D6 — whose shapes ADR-029 D3 names as successor designs for msb token selection — the implication chain has no successor; it described flags that no longer exist. The mechanical invalidation check below is unrunnable for the same reason: no `--no-ssh-config` flag, no `rc.ssh-config` label, and no `docker inspect` runtime after the msb migration. To opt out of the implication (forward agent off, but ssh-config on), pass `--no-forward-ssh --ssh-config` explicitly.
...
- ADR-014 D2 (non-interactive SSH posture: `BatchMode=yes`, pinned `known_hosts`, `StrictHostKeyChecking yes`): `BatchMode`/`StrictHostKeyChecking` defended at the translation layer (D2 transform 5, narrowed). `UserKnownHostsFile`/`GlobalKnownHostsFile` defense moves from translation layer to mount layer per [ADR-022](ADR-022-ssh-allowlist.md) — the rewrite was bypassable via `-o`; the filtered mount is not.
