...
**Status:** **RETIRED WHOLE — 2026-09-15, [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2** (human-ratified in-pane over `rip-cage-ely4`'s two sittings, 2026-09-14/15; the FIRM D2/D3 mutation this implies carries the human's explicit in-pane sign-off). Every decision here retires, D1 through D10. The project's whole config becomes **one native msb `--conf` file**, produced by the `cage-config` skill from a shipped template and carrying its lists in full — so there is nothing left to merge, no second schema to version, no provenance to reconstruct across layers, and no host-side write verbs. What each decision's problem becomes: D1/D2/D3/D9's two-file substrate, union-merge, `!replace` tag and version rules all dissolve (msb replaces lists rather than merging them, which is exactly why a second file buys nothing); D4's provenance view dissolves with the layers; D4a's symlink-follow survives as computed-at-`rc up` mounts, not a config field; D5's absent-files case is now "no config file, no cage"; D6's `session.multiplexer` re-homes to the boot descriptor ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D4); D7's read-only-in-cage property is **honored by a stronger rule** — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D5(a) puts every composition input host-side, outside any cage mount, so the file a prompt-injected agent would edit is not mounted at all rather than mounted read-only; D8's write verbs are deleted with `rc config`; D10's parked profiles question dies with the schema. Prior status: Proposed (revised 2026-07-21 — converge-on-up flipped to default-on: plain `rc up` converges reload-eligible drift on stopped cages by default with a loud announce, `--no-reload` opts out, `RC_UP_CONVERGE` retired; human-signed-off (Jonatan), rip-cage-y0u0. Prior revision 2026-07-14 — v2 merge/version model: `!replace` tag, schema version {2}, four-source provenance, write verbs, migration posture, profiles deferred; FIRM D2/D3 mutation human-signed-off (Jonatan) + 3-round adversarial design review, rip-cage-tsf2.10. Prior revision 2026-07-01 — D7 config ro-mount-by-default, rip-cage-cw51)
**Date:** 2026-05-11
...
- Self-service loop with the human as the approval step (D7/D8): the in-cage agent surfaces the request in prose; the human (or host-side assistant) applies it via `rc config add/set` or a hand edit; the next plain `rc up` of the stopped cage converges by default (loud announce, `--no-reload` opts out), or `rc reload` converges a running cage now. No hidden env var.
...
- **Recreate vs resume (msb era):** msb net rules have no live-mutation path, so config changes to create-time state (`network.allowed_hosts`, `auth.credentials`) apply via cold-recreate — `rc reload`, or `rc up` itself: on a stopped cage with reload-eligible drift, plain `rc up` converges by **default** (loud announce naming what survives; `--no-reload` opts out; `RC_UP_CONVERGE` retired; flipped from opt-in 2026-07-21, rip-cage-y0u0). RUNNING cages are never auto-recreated (ADR-029 D4; reload-eligible set is `network.allowed_hosts` post-D9). The drift hint compares live config against the applied snapshot (`config-applied.json`) and names the applying command. Manifest-egress drift is "requires rebuild," never reload-drift (D4).
