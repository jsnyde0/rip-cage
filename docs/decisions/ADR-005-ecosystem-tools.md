...
### D13: A composed floor-guard declares assert-present via `floor_assert`; the default image asserts its declared safety-stack

**Firmness: FLEXIBLE** (added 2026-06-24, rip-cage-wlwc.6)

D9 makes command-string guards (DCG, ssh-bypass) composable default-on **recipes**, and D12 keeps `rc` from ever naming a specific optional tool. The gap that leaves: a composed guard silently *dropped* from a default/bundled image (a broken recipe, a lost manifest entry) is invisible — init and the safety-stack test INFO-skip when a guard is absent (correct for a deliberately-minimal cage), so a *default* image that lost its dcg or ssh-bypass would skip rather than fail. That is the fail-open-skip / firewall-rule-presence-not-enforcement class (`bd memories rip-cage-firewall-rule-presence-not-enforcement`).

`floor_assert: <guard-id>` is an **optional manifest field on a TOOL entry** declaring "this composed tool is a safety-floor guard that MUST be present+active in the built image." The mechanism is tool-agnostic end to end:

- The codegen collects every `floor_assert` value (a guard-id string) and bakes a root-owned declaration `/etc/rip-cage/safety-stack-asserted` (one id per line; **file *and* parent dir `root:root`**) into the image. `rc`/codegen copies the strings verbatim — it never interprets `"dcg"`/`"ssh-bypass"`, so D12's "`rc` names no tool" invariant holds.
- The in-cage safety-stack test reads the declaration; for each listed guard-id it **FAILS LOUD (not INFO-skip)** if that guard is absent/inactive. A guard *not* listed → INFO-skip stays (a deliberately-minimal cage asserts nothing and remains valid). File present-but-empty/unparseable → **fail-closed**; file absent → minimal cage (trustworthy because the root-owned dir means an agent cannot have erased it — ownership-not-presence, `bd memories unix-dir-ownership-is-the-write-gate-not-file-ownership`).

So *which* guards a default image guarantees is **DATA in the manifest that composed it** (`dist/default-tools.yaml` declares `floor_assert: dcg` on the dcg-wiring entry and `floor_assert: ssh-bypass` on the ssh-bypass-hook entry), not a hardcoded list in `rc`. The default/published image asserts its declared safety-stack present+active; a custom cage asserts exactly what it declared.

**Rationale:** closes the silent-guard-loss gap for default cages while preserving the composable seam — the only knowledge of specific guards lives in the manifest (data) and the safety-stack test (which legitimately knows the guards it tests), never in `rc`. Validated end-to-end on a real dist-composed cage (rip-cage-wlwc.6 Tier-2: the baked declaration is root-owned and lists both guards; removing a listed guard flips the test RED).

**Alternatives considered:**

| Approach | Rejection |
|---|---|
| Hardcode "default image has dcg + ssh-bypass" in `rc`/test | `reasoned:` violates D12 — `rc` would name specific optional tools; the blessed-set regression vector. |
...
| Bool `floor_assert: true` keyed off entry name | `direct:` rip-cage-wlwc.6 — dist guard entries are split (`dcg` binary, `dcg-wiring` wrapper, `ssh-bypass-hook`); a bool can't name the canonical guard-id the safety-stack test maps to a check, so the field carries the guard-id string. |

**What would invalidate this:** a guard whose "present+active" cannot be cheaply asserted in-cage (forcing per-guard probe logic the test cannot host), or a validated need to assert guards *across* cages (which D8 forecloses).
