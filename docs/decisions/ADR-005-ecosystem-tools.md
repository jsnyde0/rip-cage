...
**Status:** Proposed (revised 2026-07-02 — **D7** schema evolved [rip-cage-px6v]: the TOOL archetype gains an optional agent-context `init` (boot-time) hook, generalizing to the plain TOOL the boot-time contribution the DAEMON/MULTIPLEXER archetypes already have (`start` at init), so tool-specific boot logic leaves base `init-rip-cage.sh` — reinforces D12; the hook is bounded by D11's validator like every other hook. Prior: revised 2026-06-24 — added **D13** [any TOOL may declare itself `required` (optional `assert_loaded` check or codegen-synthesized `test -x <path>` presence); codegen bakes a root-owned `/etc/rip-cage/safety-stack-asserted` declaration of `<id> <base64(check)>` lines; the in-cage safety-stack test runs each baked check generically (no name→check map) and fails loud when a required tool's check fails — closing the silent-guard-loss gap for default cages while `rc` stays tool-agnostic; drop-detection decoupled from the guards' separate behavioral checks; guard-specific `floor_assert: <guard-id>` generalized in place to the per-tool property; rip-cage-wlwc.6 + rip-cage-m8zc]. Prior: revised 2026-06-17 — **D9** forbidden-touch set rewritten in place to the CONTAINMENT floor [wlwc / ADR-025 D2 / ADR-026 D1/D2]: "DCG wiring", "PreToolUse hooks", and "the ssh-bypass blocker" REMOVED from the forbidden list — command-string guards are now composable default-on **recipes** the manifest may compose, bounded by D11's validator and protected by D7's host-only property; the welded floor is containment, not a named list. Prior: revised 2026-06-15 — added **D12** [rip-cage is a composable seam, not a bundler — rc never names/bundles/blesses an optional tool; defaults ship minimal; examples live outside the binary]; **D9** clarified [session-multiplexer provider hooks are availability-payload, not interceptors]; **D11** mechanism-2 extended [the fail-closed validator bounds multiplexer provider hooks like `install_cmd`]. Discovered fixing rip-cage-kqvw [herdr leaked into the default manifest — an optional multiplexer pre-installed for every cage, dispatched via a hardcoded `case` in `rc`]. Prior: revised 2026-06-14 — D1/D3/D4 statements corrected to reality [rip-cage-hqvk]: the build-arg customization layer they described — per-tool `INCLUDE_<TOOL>` toggles, a repo-root `versions.env`, and `rc build --with/--full/--minimal` flags — was never built. The host-only manifest [D7–D11] is the realized tool-inclusion + version-pin surface; bundled-tool versions ship as hardcoded Dockerfile `ARG` defaults; per-cage selection [the `--with` role] is deferred onto the manifest [Open-decision 8]. D11 mechanism-2 binary-root-owned coverage clarified: it runs against a declarable binary path [from-source `output_path` + prebuilt `binary_path`, rip-cage-ryn6], not literally every tool. The FIRM cores of D1/D3/D4 [install-at-build-not-runtime, pin-not-`latest`, `rc`-is-the-interface] are unchanged. Prior: revised 2026-06-12 — D11 mechanism 2 adds the entrypoint-completeness clause [the fail-closed validator must be wired into every build path, not only `rc build`], discovered-and-closed via the `rc up` auto-build bypass rip-cage-buuo.6. Prior: revised 2026-06-10 — D11 adds the agent-first composability surface [generic builder stage + fail-closed validator + authoring skill]; D2 demotes cm from bundled-default to manifest worked example; D6 generalizes the from-source fallback into D11's generic stage. Prior: revised 2026-06-05 — D7–D10 add the composable host-only tool manifest)

> **Migration status (ADR-029, 2026-07-10):** This ADR is evolved by [ADR-029](ADR-029-msb-migration.md) — D9's welded-floor enumeration moves to the lockstep msb floor list (below); MEDIATOR-archetype references in D9/D11 retire at cutover per [ADR-026](ADR-026-containment-mediation-identity.md) D5's disposition; D7's tool-lifecycle sentence naming "egress proxy, ssh-agent-filter" gets an until-cutover qualifier. D12 is untouched and remains **governing**: msb primitives are floor, never blessed-optional. D1/D3 are untouched (survive). **The msb cutover has landed (S1-S14, branch `wave/s13-docs` off `msb-cutover`) — this ADR is PARTIALLY SUPERSEDED, not wholly retired.** D9's floor enumeration and the MEDIATOR-archetype references are gone per the dispositions above. But **D12 remains a FIRM, GOVERNING constraint — current, shipped behavior** (rip-cage is a composable seam, not a bundler; msb primitives are floor, never blessed-optional), and D1/D3 survive untouched. See [ADR-029](ADR-029-msb-migration.md) for the migration decisions.
...
- **TOOL** — agent-invoked; integration is reachability (binary on PATH) + declared egress + declared mounts + an **optional agent-context `init` (boot-time) hook** (added 2026-07-02, rip-cage-px6v): a one-shot setup step run *as the agent* at cage init (NOT a long-running daemon — that is DAEMON `start`), for a tool that needs cage-boot filesystem state (worked example: pi's `~/.pi/agent/extensions/` dir). This generalizes to the plain TOOL the boot-time contribution the DAEMON/MULTIPLEXER archetypes already have, so tool-specific boot logic lives in the tool's own recipe fragment rather than base `init-rip-cage.sh`. Bounded by D11's validator like every other hook. ~75% of a real ecosystem (ACFS phases 7–10).
...
- **MULTIPLEXER** — a session multiplexer the agent's interactive session runs *inside* (tmux, herdr, zellij). Declares provider hooks: `start` (launch its server/session at cage init) and `attach` (connect a client) **required**; `exec` / `new-session` / `teardown` **optional** and tiered — a provider supplies only the hooks it supports. `rc` dispatches the per-cage `session.multiplexer` selection (ADR-021 D6) to the declared `start`/`attach` hooks, replacing the former hardcoded `none|tmux|herdr` `case`; the allowed-set is derived from the baked provider registry, **not** an `rc` enum. Provider hooks are **availability-payload** (D9 multiplexer clarification), **build-time-bounded by D11's validator** (a hook may not weaken the welded CONTAINMENT floor), and ship as `examples/` providers, never a blessed set (D12). The realized mechanism (baked registry, image label, reference reader, dynamic config derivation) is documented in D9 below.

Tools are **installed at build time** (consistent with D1 — no runtime download/plugin); a daemon archetype's process is merely **started at init**, the same lifecycle rip-cage used pre-cutover for the egress proxy, ssh-agent-filter, and tmux (the egress proxy and ssh-agent-filter are retired post-cutover per ADR-029 D2/D3 — tmux and other manifest-declared daemons still follow this lifecycle). "Install = build-time, start = init-time" — D1 forbids runtime *installation*, not daemons that run.

**TOOL `init` boot-hook generalization (2026-07-02, rip-cage-px6v — schema EXPLORATORY, announce+proceed).** The boot-time contribution point (`start` at cage init) previously existed only for the DAEMON and MULTIPLEXER archetypes; the plain TOOL archetype had none — so a tool needing cage-boot filesystem state had nowhere to declare it and the logic leaked into base `init-rip-cage.sh`. The TOOL archetype gains an **optional agent-context `init` hook**: a one-shot setup command run *as the agent* at cage init (distinct from DAEMON `start`, which launches a long-running process). This completes the **three-phase per-recipe lifecycle** — *build* (`install_cmd` / `build_source`), *boot* (`init` / `start`), *launch* (`launch_args`, ADR-027 D4) — **uniformly across archetypes**, so a recipe owns its full lifecycle wiring and **base init names no specific tool** (reinforces D12; the base-init-names-pi drift is a *symptom* of the previously-missing seam, not an independent sin). Worked example + first consumer: pi's `mkdir ~/.pi/agent/extensions` — shipped as a stopgap directly in `init-rip-cage.sh` (commit e360c3b, correct + needed: herdr's boot-time `integration install pi` requires that dir to pre-exist) and **slated to relocate into pi's own recipe via this hook**; the stopgap stays until the relocation lands (reverting it re-breaks herdr+pi and does not remove the drift). **Bounded by D11's validator** on the same footing as MULTIPLEXER/MEDIATOR hooks (cannot weaken containment, PATH-shadow a safety binary, write a DCG config / workspace `.dcg.toml`, or register a bypassing interceptor). [ADR-029 D2/D5: the MEDIATOR archetype retires at cutover per ADR-026 D5's disposition — MEDIATOR machinery is deleted, not ported; MULTIPLEXER is unaffected.] **Rationale:** make boot *uniform* across archetypes rather than invent a phase; the missing seam is exactly what forced tool-specific boot logic into base infra, reproducing the D12 drift one layer down. **Alternative rejected:** keep tool boot-logic in `init-rip-cage.sh` — `reasoned:` base init naming specific tools is the herdr-leak-shape D12 drift; the empirical trigger (pi's dir-creation surfaced by rip-cage-px6v / e360c3b) showed a tool boot-step has nowhere else to live today, which is the gap this closes. **What would invalidate this:** a boot step that genuinely requires cross-recipe coordination not expressible per-recipe — then a base-init fallback returns for that narrow case, but the default stays per-recipe (mirrors D4's fragment-order-insufficiency invalidation shape).
...
> [ADR-029 D2, LANDED: the welded-floor enumeration below is the retired Docker-path list — post-cutover it is superseded by the lockstep msb floor list: the microVM boundary itself; msb default-deny egress + net rules; msb DNS default-deny; `--secret` credential non-possession (ADR-029 D5); the secret-path mount denylist (ADR-023); the `.git/hooks` read-only weld (ADR-002 D11); and the in-guest floor items (root-owned guard artifacts, scoped sudo, ro mounts). The ADR-012/ADR-022 mechanisms below are retired, not shipped. This exact list is duplicated in lockstep across ADR-005 D9 (here), [ADR-025](ADR-025-host-adoptable-dcg-policy.md) D2, and [ADR-026](ADR-026-containment-mediation-identity.md) D2 per ADR-029 D2's lockstep-edit instruction.]

The manifest can install tools, declare their egress (which unions into the allowlist but stays under the msb-runtime IOC floor, ADR-029 D2) and mounts (subject to the ADR-023 denylist), start daemons, **and compose command-guard recipes** (DCG — its former sibling ssh-bypass retired per ADR-029 D3, now ordinary default-on recipes per ADR-025 D2, not welded floor). What it can **never** touch is the **welded CONTAINMENT floor** — the agent-agnostic layers that ADR-026 D1/D2 fix as low-drift and irreducible (below is the retired pre-cutover Docker-path enumeration, kept for historical record — see the ADR-029 D2 blockquote above for the current msb-era list):

- **sudoers scope** (ADR-002 D12) and the **non-root posture**;
- the **egress destination-control + non-overridable IOC denylist + DNS exfil floor** (ADR-012 D1/D6);
- the **filesystem sandbox** (the container boundary, ADR-002 D5);
- the **secret-path mount denylist** (ADR-023);
- the **`.git/hooks` read-only weld** (ADR-002 D4);
- the **ssh known_hosts / config-mount floor** (ADR-022 D4).

These are not manifest-composable and never become "a recipe." **Command-string guards are NOT on this list anymore** (the prior text named "PreToolUse hooks, DCG wiring, the ssh-bypass blocker"): per ADR-025 D2 a command-string classifier is high-drift harness-specific policy sitting *above* the containment line, demoted to a composable default-on recipe. So the manifest MAY compose a command-guard recipe (binary via manifest + guard wiring at an agent-unwritable load path + recipe, ADR-027 D1) — but **D11's fail-closed validator bounds every hook the manifest composes**: a composed guard hook (or any hook, ADR-027's projected-extension hooks included) still cannot weaken containment, PATH-shadow a safety binary, write a DCG config / workspace `.dcg.toml`, or register a bypassing/post-approval-mutating interceptor. The manifest composes availability (including command-guard recipes); the **welded containment floor stays welded**, and **D11 is the enforcement arm** that keeps a composed hook from ever reaching the floor.

**Why the manifest composing a command-guard is safe (the two protections):** (1) **D7's host-only property** — the manifest lives host-side under `~/.config/rip-cage/`, agent-inaccessible, so the *in-cage* agent (incl. prompt-injected) can never compose or alter a guard recipe; composition is a host/`rc`-side decision. (2) **D11's validator** — a manifest-authored or prompt-pre-staged-host-side guard recipe still has every declared hook bounded at build time on the same footing as `install_cmd` and the multiplexer/mediator provider hooks; a hook that would weaken containment, shadow the guard, or register a bypassing interceptor **fails the build**. The composability is the recipe; the bound is the validator; the floor is containment.

**Consistency with D9's existing clarifications (unchanged):** the multiplexer-provider-hooks-are-availability-payload clarification, the agent-invoked-runtime-hook (agent_mail `am guard install`) out-of-scope clarification, and the session-multiplexer provider-hook framing all stand exactly as written — they describe pull-side availability payload bounded by D11. The new framing simply adds that the manifest may also compose *push-side command-guard recipes* (DCG, ssh-bypass), which are likewise bounded by D11 and which can only ever ADD restriction above the containment floor, never subtract from it (the same additive-only discipline ADR-026 D5 applies to MEDIATOR hooks). [ADR-029 D2/D5: the MEDIATOR archetype and its additive-only discipline retire at cutover — see ADR-026 D5's disposition for the deletion sweep.]

**Rationale:** The load-bearing invariant is unchanged in spirit — "adding (or composing) a tool can never become weakening the cage" — but its *boundary* is corrected. The thing that must be uncrossable is **containment** (agent-agnostic, irreducible), not a frozen enumeration that named DCG. Welding DCG into the forbidden list conflated an accident-guardrail (ADR-002 D5) with the boundary and forced bespoke per-agent floor-lock machinery (ADR-027 D1). Confining the manifest to availability-above-containment, with D11 bounding every composed hook, keeps composition safe while letting command-guards be the ordinary composable recipes they always were across exotic agents rip-cage cannot bake wiring for.

**Counter-argument (FIRM discipline):** Letting the manifest compose command-guard hooks looks like exactly the hook-registration hole the original D9 forbade — a manifest edit, or a prompt-pre-staged entry awaiting the next `rc build`, registering a hook that weakens DCG. This does not reopen the hole: (a) D7's host-only storage means the in-cage agent cannot reach the manifest at all, so there is no in-cage pre-stage path; (b) D11's validator rejects any composed hook that weakens containment, shadows the guard, or registers a bypassing interceptor, *regardless of who authored the entry*; (c) what the manifest composes is a recipe *above* the containment floor that can only ADD restriction, never subtract — the floor it cannot cross is containment, which is not manifest-composable by construction. The original D9 was over-broad in welding DCG into the forbidden list; the corrected invariant (containment is welded; D11 bounds every composed hook) is the load-bearing one.

**Alternatives considered:**

| Approach | Rejection |
|---|---|
| Keep DCG/PreToolUse-hooks/ssh-bypass in the forbidden-touch list (original D9) | `reasoned:` conflates a high-drift accident-guardrail (ADR-002 D5, ADR-026 D2) with the irreducible containment boundary; forced bespoke per-agent floor-lock machinery (ADR-027 D1) and forbade composing a command-guard for exotic agents rip-cage cannot bake wiring for. The uncrossability that mattered (containment + in-cage-unwritable guard wiring) is preserved without the frozen list. |
| Let the manifest touch the containment floor (sudoers/non-root/egress-IOC/fs-sandbox/secret-denylist/git-hooks/known_hosts) | `direct:` that is the one move that erases the UVP (ADR-026 D1) — a composition surface that could register/alter containment could disable it. |
| Compose command-guard recipes but WITHOUT D11 bounding their hooks | `direct:` an unbounded composed hook is exactly the weaken-DCG vector; D11's fail-closed validator is the non-skippable enforcement arm (D11 mechanism 2, FIRM) that makes guard-recipe composition safe. |

**What would invalidate this:** D11's validator proving unable to bound a real composed-guard-hook vector against weakening containment (would harden the validator, not drop the recipe model); OR a containment layer turning out to genuinely require per-agent variation (would mean it is not agent-agnostic floor after all — see ADR-026 D2 invalidation).
...
D9 makes command-string guards (DCG, ssh-bypass) composable default-on **recipes**, and D12 keeps `rc` from ever naming a specific optional tool. The gap that leaves: a composed tool silently *dropped* from a default/bundled image (a broken recipe, a lost manifest entry) is invisible — init and the safety-stack test INFO-skip when a tool is absent (correct for a deliberately-minimal cage), so a *default* image that lost its dcg or ssh-bypass would skip rather than fail. That is the fail-open-skip / firewall-rule-presence-not-enforcement class (`bd memories rip-cage-firewall-rule-presence-not-enforcement`).

The assert-present property is a **generic per-tool field, not a guard category** — any TOOL entry may declare itself `required` (the guards are the first consumers, not a special-cased class; this is the wlwc collapse-to-generic thesis applied to the safety floor). A TOOL entry may carry:

- `required: true` — "this tool MUST be present+loaded in the built image; fail the cage loud if its check fails." Generic; `rc`/codegen never interpret a tool category.
- `assert_loaded: "<check cmd>"` (optional) — a **load/presence-grade** single-line shell check run verbatim per required tool (e.g. a root-owned binary/hook file exists). It is *not* a behavioral probe.
- **Codegen-synthesized presence** — when `required: true` and no `assert_loaded`, codegen synthesizes `test -x <path>` from the tool's already-declared runtime `binary_path` (or `build_source.output_path`) and bakes that as the check. The manifest path stays single-source (the baked check is a build-time projection, like `install_cmd`→RUN); no runtime divorced copy; the lib has zero path knowledge.

The mechanism is tool-agnostic end to end:

- Codegen bakes a root-owned declaration `/etc/rip-cage/safety-stack-asserted` (one `<entry-id> <base64(check-cmd)>` line per `required` tool; **file *and* parent dir `root:root`**) into the image. `rc`/codegen reads manifest **DATA** (`required` / `assert_loaded` / the declared path) and copies/synthesizes the *check* — it never interprets `"dcg"`/`"ssh-bypass"` or any tool name, so D12's "`rc` names no tool" invariant holds.
- The in-cage safety-stack test reads the declaration and, **for each line, decodes and runs the baked check with NO name→check map** — a non-zero exit **FAILS LOUD (not INFO-skip)**. A tool *not* listed → INFO-skip stays (a deliberately-minimal cage asserts nothing and remains valid). File present-but-empty/unparseable, or a base64-decode failure → **fail-closed**; file absent → minimal cage (trustworthy because the root-owned dir means an agent cannot have erased it — ownership-not-presence, `bd memories unix-dir-ownership-is-the-write-gate-not-file-ownership`). Because the test runs baked DATA, **adding a new `required` tool needs zero `rc` and zero test/lib edits** (rip-cage-m8zc acceptance #6, proven by the SI5 generic flip against an arbitrary synthetic required tool).

**Drop-detection is decoupled from behavioral-correctness.** The generic `required` mechanism is *presence/load-grade* drop-detection (did a declared-required tool vanish). The guards' rich *behavioral* checks (the dcg deny-probe, the hostile-`/workspace/.dcg.toml` anti-subversion proofs) stay **single-source in the safety-stack test** and run-when-binary-present, independent of the asserted-file — they catch *misconfiguration*; the generic mechanism catches *drop*. The generic lib never references a guard by name and never copies a behavioral probe into manifest data (avoiding the divorced-copy false-green class, `bd memories in-cage-guard-probe-false-fail-shapes`).

**Two-layer drop coverage:** the in-cage per-tool mechanism catches a *partial* drop (entry kept, the tool's check fails). The **Tier-2 `RC_E2E` release probe** catches a *full* manifest-entry drop (the whole entry removed ⇒ no line baked ⇒ in-cage INFO-skips it) by asserting the baked file lists the default image's expected required-set {dcg, ssh-bypass}. A release-gate **test** may legitimately know what the published image ships (D12 governs `rc` *code*, not tests); the in-cage `rc test` path stays data-driven and hardcodes nothing.

So *which* tools a default image guarantees is **DATA in the manifest that composed it** (`dist/default-tools.yaml` declares `required: true` + an `assert_loaded` presence check on the dcg-wiring and ssh-bypass-hook entries), not a hardcoded list in `rc`.

**Rationale:** closes the silent-guard-loss gap for default cages while preserving the composable seam *and* the wlwc collapse-to-generic thesis — the assert-present property is a field any tool carries, not a re-smuggled "guard" category; the only knowledge of specific guards lives in the manifest (data) and the safety-stack test's separate behavioral checks (which legitimately know the guards they test), never in `rc`. Validated end-to-end: Tier-1 SI5 (generic flip RED-on-absent through the real shared lib, name-free) + Tier-2 SE5 (real dist-composed cage: the baked declaration is root-owned and lists both required tools; removing one flips the test RED) — rip-cage-wlwc.6 (original) + rip-cage-m8zc (generalization).

**Alternatives considered:**

| Approach | Rejection |
|---|---|
| Hardcode "default image has dcg + ssh-bypass" in `rc` code / the in-cage data-driven `rc test` path | `reasoned:` violates D12 — `rc` would name specific optional tools; the blessed-set regression vector. The in-cage path reads the baked manifest DATA instead. (A release-gate **Tier-2 probe** legitimately hardcodes the default set — a *test* may know what the published image ships; that is not `rc` code naming a tool.) |
| Image LABEL instead of a baked file | `reasoned:` a label needs host-side `docker inspect` + env-injection into `docker exec` to reach the in-cage test; a root-owned baked file is in-cage self-contained and works for `rc test` and manual in-cage runs alike. |
| Bare bool keyed to a per-tool check the test hardcodes (`floor_assert: true` / `required: true` with no check carrier) | `direct:` rip-cage-wlwc.6/m8zc — a bare bool carries no check, so the test needs a tool-name→check map (a lib edit per tool, failing the zero-edit thesis). The fix is to pair the bool with a DATA-borne check (`assert_loaded` or codegen-synthesized presence), removing the name→check map entirely. |
| Opaque guard-id string the test maps to a check (`floor_assert: <guard-id>`, the shipped wlwc.6 form) | `direct:` rip-cage-m8zc — re-smuggles a "guard" category into the generic TOOL entry (the test must know each guard-id's check), contra D12's spirit and the wlwc collapse-to-generic thesis; superseded by generic `required` + a data-borne check, which fires from manifest data with zero `rc`/lib edit. |
...
- [ADR-025 Host-Adoptable DCG Policy Layer](ADR-025-host-adoptable-dcg-policy.md) — D2 (revised 2026-06-17): DCG is a composable default-on **recipe** the manifest may compose (no longer a welded-floor item D9 forbids), with its config floor still additive-only/uncrossable for whoever runs it and D11 bounding every composed guard hook; D5 validate-by-parsing (D9, D10).
