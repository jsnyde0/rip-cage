...
Tools are **installed at build time** (consistent with D1 — no runtime download/plugin); a daemon archetype's process is merely **started at init**, the same lifecycle rip-cage already uses for the egress proxy, ssh-agent-filter, and tmux (until the msb cutover release lands; both the egress proxy and ssh-agent-filter retire per ADR-029 D2/D3). "Install = build-time, start = init-time" — D1 forbids runtime *installation*, not daemons that run.
...
> [ADR-029 D2: EVOLVED — the welded-floor enumeration below is superseded at cutover by the lockstep msb floor list: the microVM boundary itself; msb default-deny egress + net rules; msb DNS default-deny; `--secret` credential non-possession (ADR-029 D5); the secret-path mount denylist (ADR-023); the `.git/hooks` read-only weld (ADR-002 D11); and the in-guest floor items (root-owned guard artifacts, scoped sudo, ro mounts) (shipped floor until cutover: the ADR-012/ADR-022 mechanisms, retiring per ADR-029 D2). This exact list is duplicated in lockstep across ADR-005 D9 (here), [ADR-025](ADR-025-host-adoptable-dcg-policy.md) D2, and [ADR-026](ADR-026-containment-mediation-identity.md) D2 per ADR-029 D2's lockstep-edit instruction.]

The manifest can install tools, declare their egress (which unions into the allowlist but stays under ADR-012's non-overridable IOC floor) and mounts (subject to the ADR-023 denylist), start daemons, **and compose command-guard recipes** (DCG, ssh-bypass — now ordinary default-on recipes per ADR-025 D2, not welded floor). What it can **never** touch is the **welded CONTAINMENT floor** — the agent-agnostic layers that ADR-026 D1/D2 fix as low-drift and irreducible (until the msb cutover release lands; the shipped Docker-path enumeration below remains load-bearing until then):
