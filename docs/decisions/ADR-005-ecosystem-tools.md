...
**Rationale:** The load-bearing invariant is unchanged in spirit — "adding (or composing) a tool can never become weakening the cage" — but its *boundary* is corrected. The thing that must be uncrossable is **containment** (agent-agnostic, irreducible), not a frozen enumeration that named DCG. Welding DCG into the forbidden list conflated an accident-guardrail (ADR-002 D5) with the boundary and forced bespoke per-agent floor-lock machinery (ADR-027 D5). Confining the manifest to availability-above-containment, with D11 bounding every composed hook, keeps composition safe while letting command-guards be the ordinary composable recipes they always were across exotic agents rip-cage cannot bake wiring for.
