...
1. **Credentials are discovered where you keep them, and never enter the VM.** Today that is the Claude login pulled from the macOS keychain and handed to msb's `--secret`; generalizing the discovery to other stores is roadmap, not claim.
   ...
   **Realized-vs-charted note, 2026-09-17 (`rip-cage-ely4.14`, ruled by `brain:rip-cage` on a driver raise).** As shipped at `f5af2b9`, `rc auth` pulls the Claude login from the macOS keychain into `~/.claude/.credentials.json` and `rc up` mounts that file into the guest — **possession**: the token enters the VM. The `--secret` path is a separate mechanism, and it resolves its value from `$XDG_CONFIG_HOME/rip-cage/secrets/<NAME>` or the host environment, never from the keychain. The keychain→`--secret` bridge this decision describes is therefore **charted, not shipped**; it is `rip-cage-ely4.7.17` (spike first), sequenced after the D7 dogfood gate. Until it lands, the README and `docs/reference/` describe the file mount, credit `--secret` only for credentials the operator nominates, and make no non-possession claim for the Claude login.
...
**Realized-vs-charted note, 2026-09-17 (`rip-cage-ely4.14`, ruled by `brain:rip-cage` on a driver raise).** The right-hand column's "keychain→`--secret` credential bootstrap" cell names a bridge the code does not have. As shipped at `f5af2b9`, `rc auth` pulls the Claude login from the macOS keychain into `~/.claude/.credentials.json` and `rc up` mounts that file into the guest — **possession**: the token enters the VM. The `--secret` path is a separate mechanism, and it resolves its value from `$XDG_CONFIG_HOME/rip-cage/secrets/<NAME>` or the host environment, never from the keychain. The keychain→`--secret` bridge this decision describes is therefore **charted, not shipped**; it is `rip-cage-ely4.7.17` (spike first), sequenced after the D7 dogfood gate. Until it lands, the README and `docs/reference/` describe the file mount, credit `--secret` only for credentials the operator nominates, and make no non-possession claim for the Claude login.
...
- **`rc auth`** — pulls the Claude login from the macOS keychain, and standalone re-applies a refreshed token to an existing cage without the operator recreating it by hand.
  ...
  **Realized-vs-charted note, 2026-09-17 (`rip-cage-ely4.14`, ruled by `brain:rip-cage` on a driver raise).** This bullet and `rc up`'s "keychain→`--secret` bootstrap" above both describe the bridge rather than what runs. As shipped at `f5af2b9`, `rc auth` pulls the Claude login from the macOS keychain into `~/.claude/.credentials.json` and `rc up` mounts that file into the guest — **possession**: the token enters the VM. The `--secret` path is a separate mechanism, and it resolves its value from `$XDG_CONFIG_HOME/rip-cage/secrets/<NAME>` or the host environment, never from the keychain. The keychain→`--secret` bridge this decision describes is therefore **charted, not shipped**; it is `rip-cage-ely4.7.17` (spike first), sequenced after the D7 dogfood gate. Until it lands, the README and `docs/reference/` describe the file mount, credit `--secret` only for credentials the operator nominates, and make no non-possession claim for the Claude login. *(Whether msb applies a secret change to a live sandbox or only on restart is still open — spike `rip-cage-ely4.16` Q5. The verb exists either way; only how quiet the re-apply is depends on the answer.)*
