...
**(a) Composition inputs live host-side, outside any cage mount.** The Dockerfile, the boot descriptor and the msb config are all authored where the caged agent cannot reach them. This was the manifest's real security property, and it is preserved as a *location* rule rather than a format.
