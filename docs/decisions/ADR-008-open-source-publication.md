...
**Staleness check (added post-v0.2.0):** The Dockerfile bakes `org.opencontainers.image.version=$RC_VERSION` (via `ARG RC_VERSION` + `LABEL`) at the very end of the file so version bumps don't invalidate upstream layer cache. `rc up` calls `_image_is_current` before the provisioning gate: if the label is missing, `<no value>`, or mismatches `RC_VERSION`, the local image is treated as stale and `_pull_or_build` is invoked — same code path as a missing image. Any local `rip-cage:latest` lacking the version label — including pre-GHCR local builds and the unlabeled v0.2.0 GHCR image alike — is treated as stale; users see a one-time re-provision after upgrading to the labeled-image release. If `RC_VERSION` is `"unknown"` (VERSION file absent, e.g. malformed checkout), the staleness check is skipped and the local image is treated as current. `--dry-run` on a stale image reports the would-pull-or-build message (same as missing-image).
