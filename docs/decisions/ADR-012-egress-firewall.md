...
A load-bearing enabler: mitmproxy must run with `connection_strategy=lazy` so it answers the reserved marker locally without an upstream connect (the default `eager` opens upstream first, defeating I1 against an unroutable target). See bd memory `mitmproxy-selftest-probe-inversion-lazy-connect`.
...
**What would invalidate this:** mitmproxy loses the ability to answer a request locally before upstream (so the reserved marker can't be generated without a round-trip) — I1 breaks and the classification needs rework; the harness must assert the marker arrives without an upstream round-trip.
