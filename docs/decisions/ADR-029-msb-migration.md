...
**Evolved in place 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D3** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15): **the repair loop is unchanged and is now one of rip-cage's three carrying opinions** ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D1, opinion 2: a blocked host becomes a one-line fix). Curated default allowlist, port-tight rules, denial mining into a fix-hint via `rc doctor` — all stand. **Only the verb changes: `rc reload` folds into `rc up`.** A *stopped* cage is recreated against the current config on a plain `rc up` (the converge-on-up behavior this decision already shipped), and a *running* cage is recreated only on the explicit **`rc up --replace`** — never implicitly, because that kills the live session, which is this decision's own constraint and the reason the implicit path stays closed. The loop reads deny → fix the config → `rc up --replace` → retry. The property was never in the verb's name: it was always a cold recreate with host mounts and named volumes surviving, and it still is. The allowlist itself now lives in the project's native msb config rather than `.rip-cage.yaml` ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2), and `rc allowlist` is deleted — editing the file is the one way.
