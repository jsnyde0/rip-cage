...
A known msb netstack property governed all verification here on the evidence-base's measured version (msb v0.6.4): disallowed TCP connects were **fake-accepted** (connect() succeeds, zero bytes flow; :443 grabbed by the TLS interceptor then dropped; :53 answered by msb's own DNS forwarder for any IP). Every "works/reaches" claim in the evidence base rests on real bidirectional application data, never connect() success (bd memory `msb-netstack-fake-accepts-tcp-connect-not-egress`) — that evidence discipline is unaffected by the re-measurement below and remains the standing rule. **Re-measured on msb v0.6.18 (`rip-cage-6v34.9`, 2026-09-14): the fake-accept property no longer holds.** A denied destination now fails immediately instead — a denied domain fails DNS resolution, a denied IP fails at TCP connect within a couple of milliseconds — consistent with upstream msb #1365 (v0.6.10, "deny-only default-deny policies no longer create fake connect success"). See D6 below for the scope this falls under.
...
**Evolved in place 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D3** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15): **the repair loop is unchanged and is now one of rip-cage's three carrying opinions** ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D1, opinion 2: a blocked host becomes a one-line fix). Curated default allowlist, port-tight rules, denial mining into a fix-hint via `rc doctor` — all stand. **The mechanism is unchanged; three surfaces around it move.** First and mainly, the verb: **`rc reload` folds into `rc up`.** A *stopped* cage is recreated against the current config on a plain `rc up` (the converge-on-up behavior this decision already shipped), and a *running* cage is recreated only on the explicit **`rc up --replace`** — never implicitly, because that kills the live session, which is this decision's own constraint and the reason the implicit path stays closed. The loop reads deny → fix the config → `rc up --replace` → retry. The property was never in the verb's name: it was always a cold recreate with host mounts and named volumes surviving, and it still is. The allowlist itself now lives in the project's native msb config rather than `.rip-cage.yaml` ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2), and `rc allowlist` is deleted — editing the file is the one way.
...
1. **Curated default allowlist** shipped in config, so fresh cages are not denial whack-a-mole. Seed from `rip-cage-1ujn`: `api.anthropic.com:tcp:443` is the only host a basic `claude -p` turn *requires*; `mcp-proxy.anthropic.com` and the datadog intake are attempted-but-nonblocking (include for denial-log-noise-free defaults). Contents finalized at decompose.
   ...
   **Port-tight default (revised 2026-07-21, `rip-cage-mzu6`; human sign-off jsnyde0 on the bead):** the config→msb-flags generator scopes each default floor host to **`allow@<host>:tcp:443`**, not host-wide all-ports `allow@<host>` (`cli/lib/msb_flags.sh`, unit test T17). *Counter to the prior all-ports default:* host-wide port openness on an allowed host was the generator's initial emission, never a deliberated choice; :443-scoping is strictly tighter, and spike `rip-cage-uuh9` (`tests/spike-uuh9-port443.sh`, macOS/HVF, msb 0.6.4, 17/17 probes, verified 2×) proved it costs the *entire* live default set **nothing** — a real `claude -p` turn completed with zero denials; `github.com`/`api.github.com` returned real bodies; `doltremoteapi.dolthub.com` completed its TLS+HTTP round-trip — while closing a real wrong-port hole (`example.com:80` provably blocked under :443-scoping, triangulated against an all-ports baseline and a same-cage :443 control). The tighter default dominates the looser one on evidence, so the FIRM default flips. Design constraints:
   ...
   - **C1 observability caveat.** A wrong-port block on an *allowed* domain emits **no** msb log line on any source (trace level, msb 0.6.4 and 0.6.18 alike — a log-visibility gap, not a client-symptom fact) — it drops at the NIC after DNS resolves fine, so the deny→fix→reload fix-hint miner (`_msb_denied_domains_from_trace_log`, DNS-domain-keyed) is **blind** to it. Through msb 0.6.9 it stayed self-diagnosable at the point of failure by contrast: the client got an immediate ~12ms connection-refused (curl rc=7), **not** the fake-accept hang a full host denial produced. **As of msb 0.6.18 (`rip-cage-6v34.9`) that contrast is gone** — a full host denial also refuses immediately (DNS failure for a denied name, connect failure for a denied IP) — so an instant client-side failure no longer distinguishes a wrong-port block from a full host denial; the fix-hint miner's absence of an entry for an already-allowlisted domain is the remaining tell (`cli/doctor.sh`'s `posture` NOTE). Closing the *proactive* fix-hint gap is descoped to follow-up bead `rip-cage-ffmc` (load-bearing only now that this lands).
...
2. **deny→fix→reload repair loop:** rc tails trace-level DNS-denial lines for fix-hints; apply = **snapshot-amend** (`msb run --snapshot ... --net-rule <amended>`, 0.783s, overlay preserved — default for cages with overlay state) or **cold-recreate** (0.303s, mount-only cages); the agent session resumes from host-mounted state. Proven honest wall-clock including agent relaunch: 6.085s, 94% of it Claude cold-start, msb lifecycle 0.363s (`rip-cage-1ujn`).
   ...
   **LANDED extension (`rip-cage-syzk`, commit `ac5a10a`, 2026-07-29): image staleness is a SECOND recreate trigger, stopped-cages only.** The recreate above was reachable only from a reload-eligible config diff, so a rebuilt image stranded stopped cages behind `rc up`'s stale-image refusal with volume-destroying `rc destroy` as the only documented repair (live incident: the operator's `code-personal` cage). Because the mechanism is *already* a volume-preserving cold-recreate, the repair is the same recreate under a different trigger — `rc reload` is now "re-materialize this cage against current inputs (config **and** image)". Two scoping rules bound it, and both are load-bearing for anyone extending this later:
   - **Stopped cages only.** The image-drift comparator is called *only* on the not-running branch, so the running path through `cmd_reload` is unchanged. A running cage is still never auto-recreated (recreating one kills the live agent session's guest processes) — the operator stops it first, and the messages say so.
   - **Exactly one gate relaxes: the running-gate**, and only from "exit 2 always" to "exit 2 unless (msb-Stopped AND digests differ)". The applied-config-snapshot gate, the refuse-loud on non-reload-eligible config paths, the transcript-loss guard (`rip-cage-aa4t`), the IOC-egress check and the reload lock all run unchanged on both trigger paths. In particular, image drift does **not** license applying a config change reload would otherwise refuse — that relaxation was argued and rejected as diff-without-confirm, the shape [ADR-021](ADR-021-layered-rip-cage-config.md) D7 already rejected.

   **LANDED extension (`rip-cage-7yvy`, commit `268d69a`, 2026-09-05): the same invariant reaches `rc up`'s present-branch, as an auto-resync.** The stopped-cage comparator above handles a *docker* image that moved under a cage. `rc up` has the mirror case on its image-present path: msb's local cache holding the SAME image name with DIFFERENT layer content than docker's store (`_msb_image_layer_drift_status` status 1), which until this date only warned, forever, on every `rc up`. Ruled option (a): `rc up` now msb-loads docker's image over the stale cache entry and says so, because `rc build` is the build authority and the image-*absent* path already enforces "the cage boots what `rc build` built" by loading — the present-content-differs case is the same invariant one branch over. Scoping rules that bound it: the resync is wired at `rc up`'s call site, not inside the shared drift emitter, so `rc build`'s warn-only posture is unchanged; the exit code is unchanged on every arm; a failed load falls back to the pre-existing loud status-3 warning (`rip-cage-528o`). Cross-reference for anyone extending drift handling: two entry points, one invariant.

   The graceful-stop corollary below binds here too: the stop stays **unconditional** before the remove even when the cage reads as already stopped, because the state read and the recreate are seconds apart and the reload lock does not exclude a concurrent `rc up` — skipping it could send `msb remove --force` at a guest that started in that window.
...
**The declarative surface moved 2026-09-15, the mechanism did not ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2, `rip-cage-ely4.9`).** `auth.credentials` retired with the rip-cage config schema; a binding is now declared in the project's own native msb config — `hosts` as `secrets.<NAME>.allow`, `target_env` as a line in that config's `env:` block, the host-variable name as the `secrets:` key. Leaving `value:` out is load-bearing and measured: msb then resolves the real value from the host variable at boot and the guest still holds only the placeholder, so everything this decision asserts holds unchanged. `source_file` alone has no native home; `rc` preserves its no-manual-pre-export autonomy by reading `$XDG_CONFIG_HOME/rip-cage/secrets/<NAME>` at launch. Firmness unchanged.
...
**Version re-measurement (`rip-cage-6v34.9`, 2026-09-14, macOS/HVF, msb v0.6.18):** the netstack fake-accept mechanic above was pinned to the exact evidence-base version (v0.6.4) rather than to "any msb version," so a version bump was never covered by that FIRM scope in the first place. Re-measured on v0.6.18, it **no longer holds**: a denied destination now fails immediately (DNS failure for a denied domain, connect failure for a denied IP) rather than fake-accepting the connect. This is a same-platform, different-version result — it does not touch the Linux/KVM gate above, and it does not relax or reverse any decision in this ADR; it records which mechanic-version pairing the FIRM scope actually covers. The remaining mechanics in the list (TLS-intercept-drop, APFS CoW snapshot cost, LAN-IP-only delivery, virtiofs lock non-propagation) were not in scope of this re-measurement and stand as previously recorded.
...
### D8: Restart recovery — continuously-durable state only; roster resume is provider-owned *(added 2026-07-27, `rip-cage-xqi5`; human-ratified in the convergence session)*

**Firmness: FLEXIBLE** — host-level spike-proven (S1'–S4, `docs/2026-07-27-msb-spike-roster-resume.md`); candidate for FIRM once the in-cage e2e (the xqi5 impl bead) lands.

Three clauses:

1. **Continuously-durable state only.** Any mechanism recovering in-cage agent sessions across a restart — planned `rc reload` cold-recreate, VM crash/OOM, or host reboot — may depend only on state that is durable *at all times*: host mounts and named volumes. Never the guest overlay (discarded on recreate, though it happens to survive an in-place crash-restart) and never stop-time snapshots (unavailable in the crash and reboot classes). Evidence that this collapses all three restart classes onto one recovery path: the artifact delta between an ungraceful `kill -9` and a graceful stop is *zero* — only the msb status label differs (spike P3).
2. **Roster resume is the multiplexer provider's behavior**, activated by the provider's own durably-mounted state — not rc machinery. rc's involvement is exactly the mechanics that already exist: init re-runs on every boot (D4 corollary / `rip-cage-1ujn`) and registry-dispatches the provider's `start` hook; a provider recipe makes its own state durable via its manifest `mounts:` entry (ADR-005 D12 — composition, zero rc edits). Resume *policy* (whether, what, per-runtime verbs) stays in the provider/consumer per ADR-006 D7. Host-side repair of sessions the provider could not restore is likewise a consumer verb (dotpi `hand`), not rc.
3. **The MULTIPLEXER provider contract grows no resume/rehydrate hook leg.**

**Rationale:** herdr (the current worked example, never blessed) already continuously persists its roster and natively re-execs eligible agent panes after a server restart via per-runtime resume verbs (`resume_agents_on_restore`, default-on ≥0.6.7 — restore fires on the first client attach, not on server start alone); the only cage-side gaps are durability of its state dir (a `mounts:` host-dir line, with sockets relocated guest-local via `HERDR_SOCKET_PATH` — both S4-verified) and a scripted headless PTY attach to trigger restore (S4: restore fires ~10s after any client attach, incl. a scripted PTY — no human needed; resumed panes are genuinely live `claude --resume` processes). Since the provider can fully self-resume off durable state, any rc-side resume machinery would duplicate provider behavior and pull orchestration into rc against ADR-006 D7.

**Alternatives considered:**

| Alternative | Rejection |
|---|---|
| Host-side rc re-spawn loop (snapshot roster pre-reload, re-launch post-reload) | `reasoned:` covers only the planned-reload class — crash/reboot have no pre-stop hook — and pulls orchestration intelligence into rc against ADR-006 D7. |
| New optional `resume` hook leg in the MULTIPLEXER contract | `direct:` S4 spike — herdr self-resumes off its own durable `session.json` + a scripted attach; no rc-visible hook is needed. Reopen per the invalidation signal below, as an optional tiered hook in the existing ADR-005 D7 shape. |
| In-cage auto-repair watchdog off an artifact-derived roster | `reasoned:` the artifact roster over-includes (can't distinguish roster-worthy sessions from `-p` one-shots — spike P2), so a watchdog freezes roster-worthiness judgment into a daemon and races the provider's own restore. Artifact derivation stays diagnostic-only. |
| Stop-time roster snapshot as the state source | `direct:` spike P3 — crash artifacts ≡ graceful-stop artifacts, so a snapshot adds nothing the continuously-durable state doesn't carry, and is unavailable in two of three restart classes. |

**What would invalidate this:** a multiplexer provider that demonstrably cannot self-resume from durably-mounted state (reopens the optional hook-leg alternative); the in-cage e2e showing the scripted-attach interim is unworkable under the cage's init lifecycle; host-reboot evidence (`rip-cage-ji2t`) contradicting the durable-state assumption for that class. (herdr shipping a headless restore-on-start trigger *relaxes* clause 2's mechanics — retires the scripted attach — without invalidating the decision.)
...
- ADR-002 (containers — runtime reversed by D1; D11 `.git/hooks` weld survives in the D2 floor), ADR-005 D12 (composable seam — governs D2/D3/D5's floor-vs-recipe sorting; with D7's MULTIPLEXER hook shape, bounds D8), ADR-006 D7 (orchestration intelligence lives in the consumer — governs D8's provider-owned resume), ADR-007 (beads resilience — D7), ADR-010 (auth refresh — D5 exploratory seam), ADR-012 (egress engine — deleted by D2, properties re-homed), ADR-017/018/020/022 (ssh cluster — retired by D3), ADR-023 (secret-path mount denylist — survives in the D2 floor; mechanics re-verify on msb mount syntax), ADR-024 (threat model — layer re-map + D2 residual), ADR-025 (DCG floor list — lockstep), ADR-026 (containment/mediation identity — D2/D3/D4/D5/D6/D7 evolved or fired), ADR-028 (label-lock — re-binding), ADR-021 D7 (read-only in-cage config — bounds D4's landed extension: image drift never licenses applying a config change reload would otherwise refuse).
- `docs/rip-cage-on-msb-direction.md` (vision, earn-its-keep, option map incl. host-service seam capture).
- Spike evidence: `docs/2026-07-07-microvm-spike-findings.md`, `docs/2026-07-09-msb-spike-{https-git-push,lan-ip-host-service,snapshot-amend,session-resume,claude-nonpossession,beads-virtiofs}.md`, `docs/2026-07-09-task-substrate-spike.md`, `docs/2026-07-27-msb-spike-roster-resume.md` (D8: S1'–S4 roster-resume spikes) + `docs/2026-07-27-roster-resume-design.md` (D8 converged design narrative).
- Beads: `rip-cage-tsf2` (epic; alignment rounds 1–3 in notes), `rip-cage-gljd`, `rip-cage-7fqe`, `rip-cage-akg5`, `rip-cage-0n25`, `rip-cage-1ujn`, `rip-cage-cmqb`, `rip-cage-9iab`, `rip-cage-1mkq`, `rip-cage-606c` (parity/exposure probe, open), `rip-cage-o7tx` (host-service seam capture), `rip-cage-4fxg` (KVM gate, parked), `rip-cage-uuh9` (port-tight spike, D4), `rip-cage-mzu6` (port-tight default flip, D4), `rip-cage-ffmc` (wrong-port fix-hint gap, D4 C1 follow-up), `rip-cage-syzk` (image-staleness recreate trigger, D4 landed extension; follow-ups `rip-cage-eg4i`/`rip-cage-49bw` name the two populations it deliberately leaves unrepairable), `rip-cage-xqi5` (roster-resume brainstorm, D8; impl + dotpi children filed at its close).
