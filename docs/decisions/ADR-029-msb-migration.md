...
**Rationale:** herdr (the current worked example, never blessed) already continuously persists its roster and natively re-execs eligible agent panes on server restart via per-runtime resume verbs (`resume_agents_on_restore`, default-on ≥0.6.7); the only cage-side gaps are durability of its state dir (a `mounts:` line) and a scripted headless PTY attach to trigger restore (S4: restore fires ~10s after any client attach, incl. a scripted PTY — no human needed; resumed panes are genuinely live `claude --resume` processes). Since the provider can fully self-resume off durable state, any rc-side resume machinery would duplicate provider behavior and pull orchestration into rc against ADR-006 D7.
