...
**Evolved in place 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15): **the rule this decision defends is unchanged — the patterns are DATA the operator can edit, never a constant hardcoded in `rc`.** Only their home moves. `.rip-cage.yaml` and the ADR-021 substrate retire, so the patterns become a **shipped protected-paths list file** — a plain list of known credential locations (ssh, cloud, gpg, kube, env files) that rip-cage ships as default *configuration*, editable by the operator. The replace-forbidden property this decision relied on rip-cage ADR-021's merge semantics for is now structural rather than schema-enforced: there is no merge a project file could narrow the list through. **Settled 2026-09-15 (`rip-cage-ely4.9`), where this note previously flagged an open question.** The list file **does** earn the outside-every-mount rule, and [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D5(a) now names it as the **fourth** composition input alongside the Dockerfile, the boot descriptor and the msb config. `rc up` reads it only from its own install directory or the operator's host config directory, never from a path any cage config can name — a caged agent that could edit the list could delete the line protecting the thing it wants. The timing this note already ratified is unchanged: the check runs **host-side at `rc up`, before the cage exists** (D3's single-layer rule), and an unreadable list aborts before any msb call.
