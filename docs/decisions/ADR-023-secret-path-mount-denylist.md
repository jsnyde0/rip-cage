...
**Evolved in place 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15): **the rule this decision defends is unchanged — the patterns are DATA the operator can edit, never a constant hardcoded in `rc`.** Only their home moves. `.rip-cage.yaml` and the ADR-021 substrate retire, so the patterns become a **shipped protected-paths list file** — a plain list of known credential locations (ssh, cloud, gpg, kube, env files) that rip-cage ships as default *configuration*, editable by the operator. The replace-forbidden property this decision relied on rip-cage ADR-021's merge semantics for is now structural rather than schema-enforced: there is no merge a project file could narrow the list through. Be careful about what that does **not** say — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D5(a)'s host-side location rule enumerates the three *composition inputs* (Dockerfile, boot descriptor, msb config) and the protected-paths list is **not** one of them, so this note claims no location guarantee for the list file itself. What is ratified is the timing: the check runs **host-side at `rc up`, before the cage exists** (D3's single-layer rule, unchanged), and an unreadable list aborts before any msb call. Whether the list file also earns an explicit outside-every-mount rule of its own is worth settling when [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2 is implemented — flagged here rather than assumed.

**The pre-flight stays in `rc up` and gains a third leg.** Where this ADR's D1/D3 check was refuse-on-match, the rule is now: (a) **refuse** a config that mounts a listed path directly; (b) **auto-cover** a listed path found *inside* a mounted tree — an empty read-only file mount for a file, an empty tmpfs mount for a directory; (c) **abort before any msb call** if the list file is unreadable. If msb cannot express the cover for an entry, `rc up` refuses rather than proceeding — fail closed, never fail open. (b) is new reach: it covers the credential sitting inside a project you legitimately mount, which the pure denylist could only warn about. Single-layer enforcement (D3) is unchanged — this is still host-side at `rc up` only, before the cage exists.
...
**Config-write posture (revised 2026-07-01, ADR-021 D7, rip-cage-cw51):** `.rip-cage.yaml` is read-only inside the cage by default, so the agent no longer writes `mounts.allow_risky` entries in-cage — it **surfaces the request in prose** and the human (or a host-side assistant the human relays to) authors the entry host-side before the next `rc up`. This is the same human-in-the-loop trust model accepted in ADR-022 D6 (the human running `rc up` is the approval step), tightened so author-side authorship structurally prevents a hijacked agent from *burying* an `allow_risky` path in an otherwise-legitimate edit (ADR-024). Under `mounts.config_mode: rw` (opt-in) the original in-cage self-edit flow remains available. An agent that wants to bypass the denylist must persuade the human to author the entry and re-run `rc up`. This ADR does not add a hook layer for this surface — the human-in-the-loop gate that already exists for `rc up` is sufficient under the cage's blast-radius-reduction model. If we later move to auto-reload or daemon-mode mount lifecycle, this constraint must be revisited (see "What would invalidate this" below).
...
- **Default patterns file (revised 2026-06-03 — auto-seed on first `rc up`, rip-cage-j86):** The default patterns live in `rc` as the single-source helper `_config_default_global_yaml()`, consumed by both `rc install` / `rc config init` and the auto-seed path. On `rc up`, if the global config file is absent, `rc` **auto-seeds** it with the default denylist (`_config_ensure_global_seeded`, called *before* `_config_validate_or_abort`) and emits a one-line stderr notice — rather than failing loud and directing the user to run `rc install`. **Counter-argument to the prior fail-loud posture:** the earlier rule fail-louded on an absent config "consistent with ADR-014 D2." But the invariant D6 actually protects is *"a valid config must exist before the denylist matcher runs"* (the matcher fails open on a missing/malformed config). Auto-seeding the default denylist **satisfies** that invariant rather than bypassing it, because the seeded default can only *add* blocking, never widen capability — so the safe direction is to seed, not to stop. Fail-loud here bought no safety; it only forced a separate manual `rc install` step that defeated autonomous first-run (the product's core value). The invariant is still enforced two ways: (1) seeding is self-enforcing (explicit `mkdir` + non-empty-file checks that `exit 1` on failure, not relying on the caller's `set -e`); (2) seeding runs *before* config validation, so the yq-presence check still covers the freshly-seeded config — with yq absent and a config present, `rc` still fails loud (no silent degradation), consistent with ADR-021 + ADR-001. `rc install` / `rc config init` remain for re-seeding and customization. Validated end-to-end on a fresh Apple Silicon device (auto-seed byte-identical to `rc install`, idempotent, cage up, pi guard fires).
...
- `docs/decisions/ADR-030-classify-by-use-secret-posture.md` — **orthogonal sibling, not an evolution of this ADR.** The `mounts.mask` key (ADR-030 D4) masks *content already legitimately in the mounted tree* via **explicit per-path declaration** and a boot-time `ro` overmount; this ADR's `mounts.denylist` **rejects a whole mount** via **pattern-matching** at `rc up` validation. Different surface, mechanism, and merge posture (denylist is the one replace-forbidden field per ADR-021 D2; mask is visibility-reducing and narrowable via a diff-visible `!replace`). This ADR's D1/D4/D5 stand unchanged.
