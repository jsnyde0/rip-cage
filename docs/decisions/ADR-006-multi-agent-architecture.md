...
- **Which multiplexer (if any)** is a config choice — `session.multiplexer: none | <any manifest-declared provider>`, default `none` (ADR-021 D6). The allowed set is **manifest-derived, not a fixed `tmux`/`herdr` enum** — `tmux` and `herdr` are example providers (`examples/`), not blessed names in `rc` source; adding one (e.g. `zellij`) is a manifest entry with zero `rc` edits (ADR-005 D12, FIRM; reconciled 2026-06-15, rip-cage-61al). Default `none` = normal terminal semantics (close the window → the agent ends, ADR-009 D1); there is **no in-rip-cage lever** in the default. Persistence-across-disconnect and the supervisor view are capabilities you opt into by picking a multiplexer.
...
| One window per agent inside a single tmux session (cmux-tab style) | single attached view shows all agents at once | `reasoned:` collided with the session-level `rc sessions`/`--kill`/picker as that surface stood at the choice, and needed a parallel window-level surface; the single-view payoff is not load-bearing (the factory is headless, watching via cmux+events not tmux). Session granularity reused the built surface — chosen 2026-06-08. **Corrected 2026-09-14 (rip-cage-o5be):** an earlier note here read "no `sessions` verb is registered there today," which invited the reading that the collision argument never held. It did hold. `direct:` `rc sessions` shipped — registered in the pre-split monolith's main dispatch table by commit `cb8c23d` (2026-05-20, rip-cage-ike; `git show cb8c23d:rc \| grep -n 'sessions) shift'`) and removed by `6dd69e3` (2026-06-13, rip-cage-1f59.3 — this decision's own re-decision); `git show cbfdfb4^:rc` finds no `sessions)` case because the pre-`cli/`-split monolith is dated 2026-07-08, a month *after* the removal. So the rejection was sound on its date; what is gone is the surface, not the argument. Post-retirement the collision leg is moot at this layer — window-vs-session granularity is the multiplexer's to own per the re-decision above — and the rejection now rests on its second leg, the headless-factory payoff, which is unaffected |
...
- bead `rip-cage-ike` — shipped the `rc sessions` subcommand + multi-session picker (`cb8c23d`, 2026-05-20); the surface D7's "one window per agent" row was rejected against, retired three weeks later by `rip-cage-1f59.3`.
