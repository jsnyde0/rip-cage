...
| One window per agent inside a single tmux session (cmux-tab style) | single attached view shows all agents at once | `reasoned:` collides with the existing session-level `rc sessions`/`--kill`/picker (`rc:'# Main dispatch'` — the verb table; no `sessions` verb is registered there today) and needs a parallel window-level surface; the single-view payoff is not load-bearing (the factory is headless, watching via cmux+events not tmux). Session granularity reuses the built surface — chosen 2026-06-08 |
