...
- Pi drops or changes the `--no-extensions` / `-e <path>` launch flags the wrapper relies on (post-sn1h the guard is loaded explicitly, not by auto-discovery) → re-wire the launch mechanism.
