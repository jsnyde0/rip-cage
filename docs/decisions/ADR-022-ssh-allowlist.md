...
4. Concurrent reload in progress → exit 3, message pointing at the lock file. Enforced via `flock -n ~/.cache/rip-cage/<cname>/.reload.lock` around the rewrite; same lock is acquired by `rc up` resume's filter-refresh path so the two verbs cannot interleave.
...
**Drift-hint update — `_config_emit_hint` becomes path-aware.** The container's `rc.config-loaded=<sha>` label is set at create-time and is immutable per Docker semantics. After `rc reload` applies an `allowed_hosts` change, the sha label is frozen at the pre-reload value, and naive sha comparison on `rc up` resume would emit a false-positive drift warning forever. The fix: `_config_emit_hint` (rc:4004) computes a JSON-path diff between the effective config and the create-time config (recoverable from the label or by re-loading the same workspace's `.rip-cage.yaml` as the label sha witnesses); if the only differing paths are reload-eligible (today: `.ssh.allowed_hosts`), the hint changes from "destroy+up" to "`rc reload <cage>` to apply in-place" — or is suppressed entirely if `rc reload` has been run since create-time and the differing paths are reload-eligible. Implementation note: `rc reload` writes a sidecar timestamp file (`~/.cache/rip-cage/<cname>/.reload-applied`) that `_config_emit_hint` consults; absence + sha-drift = recreate-needed; presence + reload-eligible drift only = no hint (or "reload again if you've edited since"). The sidecar is host-side; not visible inside the cage; not a security primitive.
