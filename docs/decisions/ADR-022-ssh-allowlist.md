...
>
> **This decision's central promise — HOT-reload, i.e. "without `rc destroy && rc up`" and with NO container teardown — is itself RETIRED, not merely re-mechanized, and does not carry forward.** msb's `--net-rule`/`--net-default` have no live-mutation path on a running sandbox (`msb modify` carries no network parameter — confirmed live, `docs/2026-07-09-msb-spike-egress-observability.md` Q1), so the post-cutover `rc reload` (`rip-cage-rj68`, S6) is a **COLD-RECREATE**: graceful stop → remove → the same create pipeline `cmd_up` uses, against the now-current `.rip-cage.yaml`. It is still lighter-weight than a manual `rc destroy && rc up` (one verb, no manual replay of flags/labels) and the human-approval + refuse-loud properties above hold unchanged — but "hot" (no teardown, in-flight session literally uninterrupted) is gone. What survives the recreate: host mounts (workspace, `~/.claude/{projects,sessions}` — the Claude session **resumes**) and named volumes (`rc-state-*`, `rc-history-*`, `rc-mise-cache`). What is lost: only the guest's own ephemeral rootfs overlay. See [ADR-029](ADR-029-msb-migration.md) D4's landed disposition for the full accounting.]
