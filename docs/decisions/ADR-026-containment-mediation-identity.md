...
**What would invalidate this (now FIRM — forward-looking tripwires):** (1) a dependency bump (the router's CONNECT handling, or mitmproxy/iron-proxy CLI/config) breaks the `forward_to` handoff so a composed mediator no longer receives forced-through traffic; (2) the D11 validator is found to miss a floor-weakening MEDIATOR hook (the push-side floor became crossable); (3) the host-driven launch cannot start a mediator unattended on a supported substrate (agent-as-installer breaks for the launch leg); or (4) **[discharged 2026-07-03, rip-cage-seqc.5]** a live production credential flow cannot be injected through the same seam without per-service mediation creeping back into rip-cage (D4 boundary) — tested and safe: the Anthropic case injects a static `claude setup-token` via the generic `secrets` transform with zero floor mediation and zero `rc` edits (see Validation above); this tripwire re-arms only if a future provider's flow (e.g. one that *requires* in-seam OAuth-refresh state) cannot be served without floor-side per-service logic. Any of these re-opens the composition shape.
...
**The spine: config granularity matches the mount enforcement unit.** The enforcement unit is the credential *mount*, and mounts are per-tool because the credential is one artifact per tool — claude = keychain extraction + `.credentials.json` (rip-cage-t7cu re-scope: `~/.claude.json` holds no token-shaped fields — account metadata, MCP config, onboarding/trust state — so it is not part of the credential enforcement unit; under `none` it still mounts, read-only, rather than being suppressed); pi = the single `~/.pi/agent/auth.json`. Per-tool is the **terminal** grain: per-provider posture is physically impossible (one `auth.json` holds ALL pi providers; a mount cannot split below the file), so there is no finer axis to regret not having.
