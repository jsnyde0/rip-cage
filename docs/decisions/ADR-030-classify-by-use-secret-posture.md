...
**RETIRED in place 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15). **This decision is FIRM and it retires: the `mounts.mask` config key no longer exists.** It rode ADR-021's substrate, and that substrate retires whole. **The Tier-1 capability survives, split in two.** (a) **Template mount lines** — the shipped project-config template carries the mount lines that cover the usual secret files inside a mounted project, so the common case is configured rather than coded, and the operator edits one file instead of learning a key. (b) **The auto-cover half of the protected-paths rule** ([ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2b) — `rc up` covers any *listed credential path* it finds inside a mounted tree without being asked: an empty read-only file mount for a file, an empty tmpfs mount for a directory. That half is new reach, not a like-for-like move — it fires without the operator having listed the path.

The boundary this decision drew is preserved. (b) is **not** a general masking engine: it covers only paths on the shipped protected-paths list, and it fails closed — if msb cannot express the cover for an entry, `rc up` refuses rather than proceeding. Everything outside that list stays operator judgment expressed as (a), which is the "classification is agent judgment, never rc machinery" line D1 draws. D5's fail-loud-on-missing-source and D6's legible-breadcrumb properties apply to (a); (b)'s covers are empty by design and announce themselves in the launch output. **D1, D2 and D3 — the classify-by-use model, the effort gradient and the Tier-2 judgment criterion — are untouched.**

**Firmness: FIRM**

Tier 1 is expressed by a config key `mounts.mask`: a list of **workspace-relative paths** that rc boot-time-masks with `ro` overmounts (a legible-breadcrumb **single-file** overmount shadowing the real content — regular files only, because msb `--mount-file` is regular-files-only, so a declared path that resolves to a directory fails loud at `rc up` per D5; the underlying file is unreadable in-cage, the rest of the workspace stays `rw` — spike S1). It rides [ADR-021](ADR-021-layered-rip-cage-config.md)'s existing config substrate: a **list field, union-by-default** across the three-layer merge (ADR-021 D2 v2 — global floor of masks a project expands), provenance surfaced by `rc config show`, governed `config_mode: ro` in-cage by default (ADR-021 D7 — a prompt-injected agent cannot self-edit the mask list in-cage; the human authors host-side).
...
**Residual re-scoped 2026-09-15 — [ADR-031](ADR-031-opinionated-distribution-of-microsandbox.md) D2** (human-ratified in-pane, `rip-cage-ely4` sittings 1–2, 2026-09-14/15): the Tier-1 mechanism these residuals are named against has moved — see **D4's retirement note above**. One residual narrows as a result: a credential on the shipped protected-paths list that sits inside a mounted tree is now **auto-covered by `rc up` without the operator listing it**, so the "operator must remember to mask it" residual applies only to paths *outside* that list. The list is deliberately not a general masking engine, which keeps D1's line intact: classification stays agent judgment, never rc machinery. Every other residual here stands unchanged.
...
1. **Reflection defeats non-possession (observed live, not hypothetical).** In spike S2 the bound host reflected the substituted *real* value in its response body and the guest read it; the violation guard is blind (it keys on the placeholder), so a reflected real value can then ride egress to any allowlisted host. Prompt-injection-relevant (an injected agent could deliberately elicit reflection). **Probe complete (`rip-cage-x640`, 2026-07-27):** msb 0.6.4 has no response-path scrubbing — verified across CLI, skill docs, and the upstream secret handler source (`crates/network/lib/secrets/handler.rs`, outbound/request-direction only); the gap is genuine, not config-closable. Interim operational guidance landed in [docs/reference/secret-posture.md](../reference/secret-posture.md); an upstream feature-request (Daytona-style response scrubbing) is drafted and tracked in `rip-cage-yk3s`, pending human authorization to file publicly. This residual also belongs in the ADR-024 D2 threat-model text (evolved in place alongside this ADR).
