...
The same Dockerfile works as a VS Code devcontainer. This is the **primary local interface** because:
...
**`devcontainer.json`** (per-project — while the Dockerfile is shared, each project needs its own devcontainer.json for env-file paths, port forwarding, etc.):
```json
{
  "name": "Rip Cage",
  "build": {
    "dockerfile": "path/to/rip-cage/Dockerfile",
    "args": {
  ...
  "mounts": [
    ...
    "source=${localEnv:HOME}/.config/claude-code/auth.json,target=/home/agent/.config/claude-code/auth.json,type=bind"
  ],
  "containerEnv": {
...
Simple shell script for standalone containers (no VS Code):

```bash
# Start a container mounting a path
...
9. **Devcontainer compatibility** — same Dockerfile works standalone or as VS Code devcontainer
...
### Phase 1a: Devcontainer (this design)

- Dockerfile + `devcontainer.json` template + `init-rip-cage.sh`
- OrbStack as runtime (standard Docker CLI)
- VS Code devcontainer as primary local UX
- Auto mode for Claude Code (classifier + human fallback)
- DCG + `block-compound-commands.sh` as guardrails inside container
...
- `rc test` smoke test for safety stack verification
- Bind mount only — **clone mode is Phase 2**

### Phase 1b: `rc` CLI

- `rc` shell script for standalone containers (no VS Code)
- `rc up/down/ls/attach/destroy/test` commands
- tmux session management
- Same Dockerfile and safety stack as devcontainer
