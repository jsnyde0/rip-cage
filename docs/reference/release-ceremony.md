...
> **Also confirm CI is actually green on `main` before tagging** *(v0.11.0 lesson)*: a green `--host-only` run on **macOS** does NOT prove CI (**Linux**) is green for the same commit — platform-path-dependent tests can diverge (the FHS reserved-path set differs; `config_mode=ro` shadow-mount interactions differ — see `.claude/verification.md`'s Local-vs-CI divergence checklist and `bd memories release-pretag-gate-blind-to-live-ci-status`). Run:
>
> ```bash
