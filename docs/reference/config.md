...
> **Retired ([ADR-029](../decisions/ADR-029-msb-migration.md) D2/D4):** the in-cage engine this section used to describe (SNI router, DNS sidecar, iptables REDIRECT, observe-mode traffic logging, the `network.egress.mediator`/`network.http.forward_to` auto-launched-mediator seam) is **deleted**. Egress is now an msb host-side runtime primitive, carried by the cage config itself: `network.policy: none` IS msb's default-deny, and `network.allow` names the exceptions. rc generates no `--net-default` flag (it would REPLACE that allow list — `rip-cage-ely4.7.7`); it refuses a config that omits the policy, and appends a `--net-rule allow@<host>` for any host with no home in the config. There is no in-cage process to inspect or restart.

Cages boot **default-deny**: only hosts in the effective allow set are reachable at all — everything else fails immediately (a denied domain name fails DNS resolution; a denied IP fails at TCP connect within a couple of milliseconds — measured on msb 0.6.18, `rip-cage-6v34.9`; older msb <0.6.10 instead fake-accepted the connect and delivered zero bytes. The DNS-stage denial is logged at the trace level; the connect-stage one is not). The effective allow set is `network.allowed_hosts` **union** every baked tool's manifest egress (see [egress.md](egress.md)). `network.allowed_hosts` itself unions its file layers over the schema default — the "seed ∪ global ∪ project" phrasing describes *where the file content comes from* (the curated seed is auto-written content of the global file), not a distinct merge layer; the loader folds two file layers over the schema default ([ADR-021 D2](../decisions/ADR-021-layered-rip-cage-config.md)). There is no observe mode (msb logs nothing for *allowed* flows, so rebuilding it would mean rebuilding the deleted engine) — a curated default allowlist plus a fast **deny→fix→reload** repair loop replaces it. See [egress.md](egress.md) for the full workflow.
...
- **Reload-eligible drift** (only `network.allowed_hosts` differs from snapshot — it is the *sole* reload-eligible path post-schema-v2, `_RC_RELOAD_ELIGIBLE_PATHS` in `cli/reload.sh`). This hint only fires when the caller did **not** converge — either the cage is **running** (converge-on-up only applies to stopped cages) or a stopped cage's drift was skipped via `--no-reload`: `Notice: .rip-cage.yaml has reload-eligible changes since last apply: ... Run: rc reload <cname>   (or just 'rc up' again once <cname> is stopped — it converges automatically now)`
...
Most fields apply on resume; capability-changing fields that affect create-time mounts/net-rules/secrets (e.g. `auth.credentials`, `mounts.config_mode`) require `rc destroy <name> && rc up`. Converge-on-up is **DEFAULT-ON** for a stopped cage with reload-eligible drift (`network.allowed_hosts`) — a plain `rc up` cold-recreates automatically, loudly announcing what's cold-recreated, what survives, and how to opt out ([ADR-029](../decisions/ADR-029-msb-migration.md) D4). `--no-reload` opts out for one invocation (resumes stale + the hint above); `--reload` is an accepted explicit-intent synonym for the default on a stopped cage (on a **running** cage it instead prints a "not auto-recreating" notice — a plain `rc up` on a running cage stays hint-only, never recreates). `RC_UP_CONVERGE` is **retired** (no longer read). Converge-on-up and `rc reload` are **not** a general substitute for destroy+up: on a stopped cage with non-eligible drift, `rc up` falls through to a plain resume and the resume-time mount-shape guards (`mounts.config_mode`, `mounts.symlinks.*`, `auth.credential_mounts`) abort loud, themselves naming `rc destroy && rc up` as the fix (`_up_eligible_drift_paths`, `cli/up.sh`). Pure-content edits to `network.allowed_hosts` apply via `rc up` or `rc reload` — **a cold-recreate**, not a live in-place apply ([ADR-029](../decisions/ADR-029-msb-migration.md) D4; [ADR-022](../decisions/ADR-022-ssh-allowlist.md) D6's retirement note). Each consumer documents which of its fields are reload-eligible vs recreate-required vs resume-applicable. **Image staleness is the one exception** (rip-cage-syzk): a STOPPED cage whose pinned image has drifted from the current build (`rc build` ran since create/last repair) IS now `rc reload`-repairable — same volume-preserving cold-recreate, different trigger — leaving the cage running afterward; a RUNNING cage's own image drift still has no in-place repair (`rc down <name> && rc reload <name>`), and mount-shape / credential-shape drift on either state remains destroy+up-only as described above.
