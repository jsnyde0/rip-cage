...
The base image does **not** bake any PreToolUse command-guard hooks. DCG is a **composable recipe** (`examples/dcg/`) — default-on in the published image when composed, but absent in a custom minimal cage that omits it. See ADR-025 D2 and ADR-026 D2 (command-string policy is composable, not floor). DCG's former sibling, the ssh-bypass blocker (`hooks/block-ssh-bypass.sh`, `examples/ssh-bypass/`), is **deleted, not just undocumented** — it guarded ssh host-key-override flags, and the entire ssh cluster it protected retired at the msb cutover in favor of git over HTTPS + `--secret` ([ADR-029](../decisions/ADR-029-msb-migration.md) D3).
...
Claude Code runs with `bypassPermissions` enabled in `settings.json`. This means the permission allowlist doesn't gate commands. When the DCG recipe is composed, its PreToolUse hook fires on every command and provides the command-guard safety layer. Without it, the cage falls back to containment-only (the msb host/VM boundary, default-deny egress/DNS, filesystem sandbox, non-root user, secret-path denylist, and the `.git/hooks` RO weld — see [ADR-029](../decisions/ADR-029-msb-migration.md) D2 for the post-cutover floor enumeration).
