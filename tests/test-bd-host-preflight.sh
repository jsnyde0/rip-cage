...
check() {
  ...
  actual_out=$("$RC" bd-preflight-test "$beads_dir" "$dolt_mode" 2>/dev/null) || actual_exit=$?
...
# State 4: server-mode, port file stale (case B) — port 59998, nothing listening there
mkdir -p "$TMPDIR_FIXTURE/stale/.beads"
printf '59998\n' > "$TMPDIR_FIXTURE/stale/.beads/dolt-server.port"
check "stale port (case B)" 1 "stale port 59998" \
...
# State 6 (optional): server-mode, port file healthy — start a listener
HEALTHY_PORT=59997
LISTENER_PID=""
...
s.bind(('127.0.0.1', $HEALTHY_PORT))
s.listen(1)
time.sleep(10)
" &
  LISTENER_PID=$!
  sleep 0.3  # Give listener a moment to start
