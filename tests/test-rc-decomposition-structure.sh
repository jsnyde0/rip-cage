...
# ---------------------------------------------------------------------------
echo "=== (b) Function-count invariant (measured pre-split count: 193, current: 178) ==="

EXPECTED_FN_COUNT=178
...
#     of the 178 currently rc-reachable functions must be `declare -F`-
...
#     and reachable count are the SAME number (178, since rip-cage-tsf2.1's
...
# ---------------------------------------------------------------------------
echo "=== (c) declare -F reachability: all 178 rc-reachable functions defined after sourcing rc ==="
...
if [[ "$_missing_count" -eq 0 ]]; then
  pass "(c)" "all 178 rc-reachable functions are declare -F reachable after sourcing rc"
else
