...
# ---------------------------------------------------------------------------
echo "=== (b) Function-count invariant (measured pre-split count: 193, current: 179) ==="
...
#     of the 179 currently rc-reachable functions must be `declare -F`-
...
#     and reachable count are the SAME number (179, since rip-cage-tsf2.1's
...
if [[ "$_missing_count" -eq 0 ]]; then
  pass "(c)" "all 179 rc-reachable functions are declare -F reachable after sourcing rc"
else
