...
#     of the 193 pre-split functions must be `declare -F`-reachable. A loud,
#     per-name failure (not just a count) so a reviewer can see exactly which
#     module dropped/misfiled a function.
# ---------------------------------------------------------------------------
...
if [[ "$_missing_count" -eq 0 ]]; then
  pass "(c)" "all 193 pre-split functions are declare -F reachable after sourcing rc"
else
...
_coupling_result=$(bash -c '
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/output.sh"
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/docker.sh"
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/container.sh"
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/path.sh"
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/config.sh"
  # shellcheck disable=SC1090
  source "'"${REPO_ROOT}"'/cli/lib/manifest_checks.sh"
  # shellcheck disable=SC1090
