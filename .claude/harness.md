...
> - **GNU vs BSD coreutils.** `sed`/`grep`/`awk`/`date`/`stat` differ. Specifically `sed -n '…' | grep -q` returns **141 under `set -o pipefail` on GNU** (sed takes SIGPIPE when grep closes the pipe early; BSD suppresses it) → the `if` reads false even on a match. Prefer a single `awk` pass over `sed|grep` where a downstream reader can close early. (test-auth-refresh L10)
