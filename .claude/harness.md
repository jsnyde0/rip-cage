...
| Mediator credential-injection e2e (composable egress-mediator seam, ADR-026 D5) | `test-security-model-injection.sh` via **`rc test --e2e-security`** — spins real cages and runs B1-B9 (floor deny/allow baseline), O1-O2 (observe-mode), E4 (mitmproxy injection, rip-cage-ta1o.5.8), E4-ip (iron-proxy injection, rip-cage-nyst). **Requires the target mediator BAKED into the image** (`rc build` with the mediator's manifest fragment in `~/.config/rip-cage/tools.yaml`; the `rc.mediators` label must include it) — E4/E4-ip self-skip if absent, BUT the skip path currently emits `pass "SKIP"` (latent vacuous-green for un-baked runs — verify the image is baked before trusting green). Each mediator's predicate family: positive injection (sentinel echoed, liveness-gated), **negative control** (no `--mediator-env` → placeholder passes through un-injected — proves injection is load-bearing), **non-possession** (sentinel absent from `/proc/1/environ`, read as agent, PATH= liveness gate), floor-deny holds composed (real host `example.org`), and **zero rc diff** (seam tool-agnostic). Real-host flake handling: httpbin.org→httpbingo.org fallback fires on `curl_exit -ne 0 || non-200` (connection-level failures, not just 5xx) on EVERY body-asserting curl — see bd memory `rip-cage-egress-e2e-probes-need-real-hosts`. Verify green in your OWN shell, not carried from an implementer (a transient timeout makes a single run's green luck-dependent). |
