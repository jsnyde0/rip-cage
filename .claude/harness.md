...
> - **No non-reserved writable scratch dir on Linux (rip-cage-woow).** rc's symlink-mount reserved-path set is the whole FHS top-level + `/tmp` + `/home` (rc:1511-1513), and it deliberately does NOT canonicalize `/var`/`/tmp` to dodge macOS `/private/var` false-positives. On macOS `mktemp` → `/var/folders/...` (`/private/var/...`) so symlink-target fixtures are permitted; on a **Linux** runner every writable location (`/home`, `/tmp`, `/var`) IS reserved and a non-root user can write to no non-reserved top-level (`/mnt`,`/srv`,`/opt` are root-owned). A test that needs a non-reserved writable scratch dir for symlink targets therefore can only run on macOS — re-tier it to NEEDS_CONTAINER (it runs in the full local/dev suite). (test-symlink-follow.sh)
> - **mtime is a flaky idempotency proxy (rip-cage-woow).** `stat %m`/`%Y` is second-granularity; an idempotency assertion comparing mtime before/after passes on a fast box (both events in one second) and fails intermittently on a slower CI runner. Assert **content equality** (`[[ "$before" == "$(cat f)" ]]`), the true invariant — it catches a real rewrite (content changes) without the timing flake. (test-auto-seed.sh S2)
>
> **Faithful local reproduction of the CI host-only job (rip-cage-woow — the harness that ends the thrash).** The maintainer's mac cannot reproduce these axes; reproduce in a Linux container that mimics the bare ubuntu runner. rc bails inside a container via `[[ -f /.dockerenv ]]` (rc:34, no env hatch) — so **`rm -f /.dockerenv` first** to make rc behave like CI's VM. Provision to match the runner: `jq`+system `yq` at `/usr/bin` (so `PATH=/usr/bin:/bin` does NOT hide yq — the config-loader divergence), `python3` (tomllib present), `docker.io` + the mounted host socket (docker PRESENT), and **no cached `rip-cage:latest`**. Run the repo from `/work` for a quick pass. Two faithfulness gotchas that masked real failures here: (1) CI checks out under `/home/runner/...` which is itself rc-reserved — copy the repo under `/home/...` to expose `/tmp`/`/home`-reserved bugs a `/work` checkout hides; (2) CI runs as a non-root user, but a non-root user added to the `docker` group mid-`bash -c` can't reach the socket → every `rc`-touches-docker test false-fails, so for non-symlink-follow target files prefer running as root. There is no single perfectly-faithful local replica (the runner is a fat VM with many preinstalled tools) — use it to reproduce the *named* target files, and treat live CI as the authoritative full-suite gate.
...
| Pi substrate projection (ADR-027 D2, rip-cage-kstk) | **Structural/host:** `test-pi-substrate-mounts.sh` (verifies mount args, realpath resolution, denylist warn-and-skip, init symlink wiring, SYSTEM.md seam, floor-shadow prevention). **Real-cage e2e — manual only, not yet a committed test:** `rc up <workspace>` → `pi --prompt "/skill:send-it <bead>"` inside cage → assert (a) skill resolves, (b) subagent tool registered, (c) dcg-gate extension not shadowed. Signal (a) requires cage started AFTER last `dotpi install.sh` run (see bind-mount inode anti-pattern). Gated-e2e acceptance is open work (rip-cage-wlwc scope). |
| In-cage multiplexer lifecycle (`session.multiplexer: none\|tmux\|herdr`, ADR-021 D6 / ADR-006 D7) | `test-multiplexer-lifecycle.sh` — **NEEDS_CONTAINER, gated `RC_E2E=1`** (self-skips visibly otherwise). Parameterized over the multiplexer value: asserts default-`none` plain-shell + **no** mux server (enumerated procs), two independent `rc exec`/`rc attach` terminals (close-one-leaves-other), `tmux` detach/reattach survival, `rc agent`/`rc sessions` retirement across dispatch/help/schema/completions/json-allowlist, and config-isolation under each mux (herdr **gating** via `HERDR_SESSION`). herdr semantic status-view render assertion (rip-cage-w621.9 / ADR-006 D8, RC_E2E+openrouter-auth-gated): drives a brief pi agent THROUGH `herdr agent start`, polls `herdr agent list` JSON for `agent_status=working` AND `screen_detection_skipped=true` (the integration path marker — distinguishes D8 semantic integration from process-detection fallback which shows `idle`). If openrouter auth absent in cage → visible SKIP. This gap is CLOSED — no longer a skip-with-log. This is the integration Signal for the multiplexer-decouple epic; it REPLACED `test-multi-agent-levers.sh` (rip-cage-1f59.8). Spins+tears-down 3 scratch cages (cleanup trap uses realpath-resolved labels). |
...
### Drive an agent THROUGH the multiplexer attach surface (the `-p` cells above *bypass* the mux)
- **What it is:** `claude -p` / `pi -p` via `docker exec` go straight to the agent and BYPASS the multiplexer. To exercise the *composed* surface (an agent doing real work under tmux/herdr supervision while the floor holds), drive *through* the attach surface: **tmux** — `docker exec <cage> tmux send-keys -t rip-cage "bash /workspace/run-pi.sh" Enter` (write the runner to the bind-mount first to dodge multi-line quoting); **herdr** — `docker exec <cage> herdr agent start <name> --cwd /workspace -- pi --provider openrouter -p "<prompt>"` (which ALSO makes `herdr agent list` report the semantic `agent_status` working/idle — the ADR-006 D8 integration path, vs process-detection `idle`).
- **Catches:** the composition premise itself — that the mux can DRIVE a non-interactive agent (validated rip-cage-w621: tmux send-keys drives pi AND claude; herdr-start drives pi + renders status headlessly), and that the safety floor holds in the composed config. The halves each miss this intersection: `test-multiplexer-lifecycle.sh` = attach lifecycle, no agent work; `test-pi-e2e.sh` = agent, but bypasses the mux via `docker exec`.
- **Proof of agency (DP6 discriminator):** capture the agent's `--session-dir` session JSONL — a native `write` toolCall (not bash shell-redirection) proves the agent *iterated*, not merely launched a self-contained script. See `rip-cage-test-fail-prose-without-exit-silent-red` DP6.
- **Crystallized regression:** `test-multiplexer-agent-e2e.sh` (pi/tmux, RC_E2E-gated, red-on-revert) + the herdr status-view render assertion in `test-multiplexer-lifecycle.sh`. This is the dispatch rung *above* `pi -p`; it de-risks the ClawPatrol egress-harness warm-up (rip-cage-ta1o).
...
- **`source rc` (no `./`) silently sources a brew-installed `rip-cage` from PATH, not the repo copy (rip-cage-61al).** The brew cellar's `rc` is on PATH, so a bare `source rc` (or invoking `rc` by name) in a verification step resolves the installed binary — which may predate a mechanism under test and give a false "mechanism absent" signal (during rip-cage-61al re-verification, the brew rc lacked the new MULTIPLEXER archetype, reading as "not implemented" against a repo that had it). Always `source ./rc` / invoke `./rc` for in-repo verification; confirm with `which rc` + `./rc --version` before trusting a mechanism-absent reading. Inverse of the "Patched-rc without a release" gotcha above (there the repo is newer than brew; here too — the trap is the shadowing direction).
...
- **A directory bind-mount whose source is the host REAL-DIR path becomes ENOENT-on-opendir if the host tree is rm+recreated after the cage starts (rip-cage-kstk).** The inode the container holds ceases to exist; Docker does not re-resolve the path. Symptom: `drwxr-xr-x?` permissions + `opendir-ENOENT` on the mounted directory even though the host path looks intact. Fix: fresh `rc up` (started AFTER the last `dotpi install.sh` or any host operation that rm+recreates the source tree). Distinct from the single-file atomic-rename shape (`rip-cage-single-file-mount-handle-breaks-on-host-atomic-rename`). For pi substrate verification: always confirm the cage was started AFTER the last `dotpi install.sh` run before diagnosing a skill-resolution failure as a mount-wiring bug. See bd memory `rip-cage-bind-mount-real-dir-invalidated-by-host-tree-recreate`.
