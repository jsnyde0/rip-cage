...
# tests/fixtures/ripcage-testsentinel-rule.yaml).
#
...
tools:
  ...
  # ---------------------------------------------------------------------------
  # TOOL entry 2: provision the dcg engine wiring root-owned (guard + config + sentinel).
  ...
  - name: dcg-wiring
    ...
    install_cmd: ": && mkdir -p /usr/local/lib/rip-cage/bin /usr/local/lib/rip-cage/dcg/fixtures && echo '${GUARD_B64}' | base64 -d > /usr/local/lib/rip-cage/bin/dcg-guard && chown root:root /usr/local/lib/rip-cage/bin/dcg-guard && chmod 0755 /usr/local/lib/rip-cage/bin/dcg-guard && echo '${CONFIG_B64}' | base64 -d > /usr/local/lib/rip-cage/dcg/config.toml && chown root:root /usr/local/lib/rip-cage/dcg/config.toml && chmod 0644 /usr/local/lib/rip-cage/dcg/config.toml && echo '${SENTINEL_B64}' | base64 -d > /usr/local/lib/rip-cage/dcg/fixtures/ripcage-testsentinel-rule.yaml && chown root:root /usr/local/lib/rip-cage/dcg/fixtures/ripcage-testsentinel-rule.yaml && chmod 0644 /usr/local/lib/rip-cage/dcg/fixtures/ripcage-testsentinel-rule.yaml"
    egress: []
    mounts: []
YAML
